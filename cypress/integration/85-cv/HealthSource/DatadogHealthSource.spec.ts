...
describe('Configure Datadog health source', () => {
  ...
  it('Datadog metric health source should show an error toast if any metric other than current selected metic is invalid', () => {
    cy.addNewMonitoredServiceWithServiceAndEnv()
    ...
    // map metric to services
    cy.mapMetricToServices(true, false)

    cy.contains('span', '+ Manually input query').scrollIntoView().should('be.visible').click()

    cy.contains('h4', 'Add your Datadog query').should('be.visible')

    cy.get(`.ManualInputModal [name="metricName"]`).clear().type('Datadog Metric 2')

    cy.get('.ManualInputModal button[type="submit"]').should('be.visible').click()

    // cy.contains('p', 'Datadog Metric').should('be.visible').click()

    cy.findByText(/^Datadog Metric$/)
      .should('be.visible')
      .click()
    ...
    cy.findByText(/^Datadog Metric 2$/)
      .should('exist')
      .scrollIntoView()
      .should('be.visible')

    cy.findByText(/Datadog Metric 2 metric is invalid/).should('be.visible')
  })
