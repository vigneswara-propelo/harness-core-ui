...
describe('API Utils tests', () => {
  test('that isJsonResponse returns correctly with correct headers', () => {
    const Response = jest.fn().mockImplementation((url, options) => {
      const headers = {
        get: jest.fn().mockImplementation(key => {
          return options?.headers?.[key]
        })
      }
      return { clone: jest.fn(), url, headers }
    })

    const response = new Response('url', { headers: { 'content-type': 'text' } })
    expect(isJsonResponse(response)).toBe(false)
    const response2 = new Response('url')
    expect(isJsonResponse(response2)).toBe(false)
    const response3 = new Response('url', { headers: { 'content-type': 'application/json' } })
