...
import { DeployCloudFunctionRollbackStepGenOne } from '../DeployCloudFunctionRollbackStepGenOne'
...
const existingInitialValues = {
  ...
  spec: {},
  timeout: '20m',
  type: StepType.RollbackCloudFunctionGenOne
}
const onUpdate = jest.fn()
const onChange = jest.fn()

describe('DeployCloudFunctionRollbackStepGenOne tests', () => {
  beforeEach(() => {
    onUpdate.mockReset()
    onChange.mockReset()
  })
      ...
      <TestStepWidget
        ...
        customStepProps={{
          ...
          metadataMap: {
            'Step 1': {
              yamlProperties: {
                fqn: 'pipeline.stages.qaStage.execution.steps.RollbackCloudFunctionGenOne.name',
                localName: 'step.RollbackCloudFunctionGenOne.name'
              }
            },
            '20m': {
              yamlProperties: {
                fqn: 'pipeline.stages.qaStage.execution.steps.RollbackCloudFunctionGenOne.timeout',
                localName: 'step.RollbackCloudFunctionGenOne.timeout'
              }
            }
          }
        }}
