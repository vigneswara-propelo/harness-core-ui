...
import { AwsLambdaDeployStep } from '../AwsLambdaDeployStep'

factory.registerStep(new AwsLambdaDeployStep())

const existingInitialValues = { identifier: 'Step_1', name: 'Step 1', timeout: '20m', type: StepType.AwsLambdaDeploy }
const onUpdate = jest.fn()
const onChange = jest.fn()

describe('AwsLambdaDeployStep tests', () => {
  beforeEach(() => {
    onUpdate.mockReset()
    onChange.mockReset()
  })
      ...
      <TestStepWidget
        ...
        customStepProps={{
          ...
          metadataMap: {
            'Step 1': {
              yamlProperties: {
                fqn: 'pipeline.stages.qaStage.execution.steps.AwsLambdaDeploy.name',
                localName: 'step.AwsLambdaDeploy.name'
              }
            },
            '20m': {
              yamlProperties: {
                fqn: 'pipeline.stages.qaStage.execution.steps.AwsLambdaDeploy.timeout',
                localName: 'step.AwsLambdaDeploy.timeout'
              }
            }
          }
        }}
