...
import { useGetLastSuccessfulVersion, useGetLastSuccessfulVersionWithServiceV2 } from 'services/cd-ng'
...
export function useGetDigestDetailsForGithubPackageRegistryArtifact(params: Params) {
  const {
    connectorRef,
    org,
    packageName,
  ...
  const {
    data: gprV2DigestData,
    loading: fetchingV2Digest,
    refetch: fetchV2Digest,
    error: fetchV2DigestError
  } = useMutateAsGet(useGetLastSuccessfulVersionWithServiceV2, {
    body: {
