...
import { AwsCDKSynthStep, AwsCDKSynthStepInitialValues } from '../AwsCDKSynthStep'
...
const existingInitialValues: AwsCDKSynthStepInitialValues = {
  identifier: 'Step_1',
  name: 'Step 1',
  spec: {
    connectorRef: 'account.Kubernetes_Connector_2',
    image: 'aws-cdk-synth-prod:latest',
...
const awsCdkSynthRuntimeTemplate: AwsCDKSynthStepInitialValues = {
  identifier: 'Step_1',
  name: 'Step 1',
  spec: {
    ...
    exportTemplate: RUNTIME_INPUT_VALUE as unknown as boolean,
...
describe('AwsCdkSynthStep tests', () => {
  beforeEach(() => {
    onUpdate.mockReset()
    onChange.mockReset()
  })
  ...
  test('it should display correct initial values for existing step', async () => {
    ...
    expect(imageInput.value).toBe('aws-cdk-synth-prod:latest')
    fireEvent.change(imageInput, { target: { value: 'aws-cdk-synth-prod2:latest' } })
    expect(imageInput.value).toBe('aws-cdk-synth-prod2:latest')

    const optionalConfigAccordion = screen.getByText('common.optionalConfig')
    userEvent.click(optionalConfigAccordion)
    const optionalConfigAccordionPanel = screen.getByTestId('aws-cdk-synth-optional-accordion-panel')
    ...
    const exportTemplateCheckbox = queryByNameAttribute('spec.exportTemplate', container) as HTMLInputElement
    expect(exportTemplateCheckbox).toBeChecked()
    await userEvent.click(exportTemplateCheckbox)
    ...
    expect(appPathInput.value).toBe('/test/app')
    ...
    await waitFor(() =>
      expect(onUpdate).toHaveBeenCalledWith({
        ...
        spec: {
          connectorRef: 'account.Kubernetes_Connector_2',
          image: 'aws-cdk-synth-prod2:latest',
  ...
  test('it should display Runtime input as initial values for all fields in edit view', async () => {
    ...
    const { container } = render(
      <TestStepWidget
        initialValues={awsCdkSynthRuntimeTemplate}
        type={StepType.AwsCdkSynth}
        onChange={onChange}
        onUpdate={onUpdate}
        isNewStep={false}
  ...
  test('it should show errors for required fields when stepViewType is DeploymentForm', async () => {
    const { container, getByText, getAllByText } = render(
      <TestStepWidget
        ...
        template={awsCdkSynthRuntimeTemplate}
        type={StepType.AwsCdkSynth}
  ...
  test('it should show errors for required fields when stepViewType is TriggerForm', async () => {
    ...
    const step = new AwsCDKSynthStep()
      ...
      <TestStepWidget
        ...
        customStepProps={{
          ...
          metadataMap: {
            'Step 1': {
              yamlProperties: {
                fqn: 'pipeline.stages.qaStage.execution.steps.AwsCdkSynth.name',
                localName: 'step.AwsCdkSynth.name'
              }
            },
            '20m': {
              yamlProperties: {
                fqn: 'pipeline.stages.qaStage.execution.steps.AwsCdkSynth.timeout',
                localName: 'step.AwsCdkSynth.timeout'
              }
            }
          }
        }}
