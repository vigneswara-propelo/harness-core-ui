...
import { AwsCDKDestroyStep, AwsCDKDestroyStepInitialValues } from '../AwsCDKDestroyStep'
...
const existingInitialValues: AwsCDKDestroyStepInitialValues = {
  identifier: 'Step_1',
  name: 'Step 1',
  spec: {
    connectorRef: 'account.Kubernetes_Connector_2',
    image: 'aws-cdk-destroy-prod:latest',
...
const awsCdkDestroyRuntimeTemplate: AwsCDKDestroyStepInitialValues = {
  identifier: 'Step_1',
  name: 'Step 1',
  spec: {
...
describe('AwsCdkDestroyStep tests', () => {
  beforeEach(() => {
    onUpdate.mockReset()
    onChange.mockReset()
  })
  ...
  test('it should display correct initial values for existing step', async () => {
    ...
    expect(imageInput.value).toBe('aws-cdk-destroy-prod:latest')
    fireEvent.change(imageInput, { target: { value: 'aws-cdk-destroy-prod2:latest' } })
    expect(imageInput.value).toBe('aws-cdk-destroy-prod2:latest')

    const optionalConfigAccordion = screen.getByText('common.optionalConfig')
    userEvent.click(optionalConfigAccordion)
    const optionalConfigAccordionPanel = screen.getByTestId('aws-cdk-destroy-optional-accordion-panel')
    ...
    await waitFor(() =>
      expect(onUpdate).toHaveBeenCalledWith({
        ...
        spec: {
          connectorRef: 'account.Kubernetes_Connector_2',
          image: 'aws-cdk-destroy-prod2:latest',
  ...
  test('it should display Runtime input as initial values for all fields in edit view', async () => {
    ...
    const { container } = render(
      <TestStepWidget
        initialValues={awsCdkDestroyRuntimeTemplate}
        type={StepType.AwsCdkDestroy}
        onChange={onChange}
        onUpdate={onUpdate}
        isNewStep={false}
  ...
  test('it should show errors for required fields when stepViewType is DeploymentForm', async () => {
    const { container, getByText, getAllByText } = render(
      <TestStepWidget
        ...
        template={awsCdkDestroyRuntimeTemplate}
        type={StepType.AwsCdkDestroy}
  ...
  test('it should show errors for required fields when stepViewType is TriggerForm', async () => {
    ...
    const step = new AwsCDKDestroyStep()
      ...
      <TestStepWidget
        ...
        customStepProps={{
          ...
          metadataMap: {
            'Step 1': {
              yamlProperties: {
                fqn: 'pipeline.stages.qaStage.execution.steps.AwsCdkDestroy.name',
                localName: 'step.AwsCdkDestroy.name'
              }
            },
            '20m': {
              yamlProperties: {
                fqn: 'pipeline.stages.qaStage.execution.steps.AwsCdkDestroy.timeout',
                localName: 'step.AwsCdkDestroy.timeout'
              }
            }
          }
        }}
