...
import { AwsCDKBootstrapStepInitialValues } from '@pipeline/utils/types'
import { AwsCDKBootstrapStep } from '../AwsCDKBootstrapStep'
...
const existingInitialValues: AwsCDKBootstrapStepInitialValues = {
  identifier: 'Step_1',
  name: 'Step 1',
  spec: {
    connectorRef: 'account.Kubernetes_Connector_2',
    image: 'aws-cdk-bootstrap-prod:latest',
...
const awsCdkBootstrapRuntimeTemplate: AwsCDKBootstrapStepInitialValues = {
  identifier: 'Step_1',
  name: 'Step 1',
  spec: {
...
describe('AwsCdkBootstrapStep tests', () => {
  beforeEach(() => {
    onUpdate.mockReset()
    onChange.mockReset()
  })
  ...
  test('it should display correct initial values for existing step', async () => {
    ...
    expect(imageInput.value).toBe('aws-cdk-bootstrap-prod:latest')
    fireEvent.change(imageInput, { target: { value: 'aws-cdk-bootstrap-prod2:latest' } })
    expect(imageInput.value).toBe('aws-cdk-bootstrap-prod2:latest')

    const optionalConfigAccordion = screen.getByText('common.optionalConfig')
    userEvent.click(optionalConfigAccordion)
    const optionalConfigAccordionPanel = screen.getByTestId('aws-cdk-bootstrap-optional-accordion-panel')
    ...
    await waitFor(() =>
      expect(onUpdate).toHaveBeenCalledWith({
        ...
        spec: {
          connectorRef: 'account.Kubernetes_Connector_2',
          image: 'aws-cdk-bootstrap-prod2:latest',
  ...
  test('it should display Runtime input as initial values for all fields in edit view', async () => {
    ...
    const { container } = render(
      <TestStepWidget
        initialValues={awsCdkBootstrapRuntimeTemplate}
        type={StepType.AwsCdkBootstrap}
        onChange={onChange}
        onUpdate={onUpdate}
        isNewStep={false}
  ...
  test('it should show errors for required fields when stepViewType is DeploymentForm', async () => {
    const { container, getByText, getAllByText } = render(
      <TestStepWidget
        ...
        template={awsCdkBootstrapRuntimeTemplate}
        type={StepType.AwsCdkBootstrap}
  ...
  test('it should show errors for required fields when stepViewType is TriggerForm', async () => {
    ...
    const step = new AwsCDKBootstrapStep()
      ...
      <TestStepWidget
        ...
        customStepProps={{
          ...
          metadataMap: {
            'Step 1': {
              yamlProperties: {
                fqn: 'pipeline.stages.qaStage.execution.steps.AwsCdkBootstrap.name',
                localName: 'step.AwsCdkBootstrap.name'
              }
            },
            '20m': {
              yamlProperties: {
                fqn: 'pipeline.stages.qaStage.execution.steps.AwsCdkBootstrap.timeout',
                localName: 'step.AwsCdkBootstrap.timeout'
              }
            }
          }
        }}
