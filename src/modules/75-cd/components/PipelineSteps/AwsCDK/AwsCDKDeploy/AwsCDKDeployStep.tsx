...
import { AwsCDKDeployStepInputSetMode } from './AwsCDKDeployStepInputSet'
import { AwsCDKDeployStepEditRef } from './AwsCDKDeployStepEdit'
...
export interface AwsCDKDiffStepEditProps {
  ...
  onUpdate?: (data: AwsCDKDeployStepInitialValues) => void
  stepViewType?: StepViewType
  onChange?: (data: AwsCDKDeployStepInitialValues) => void
...
export interface AwsCDKDeployVariableStepProps {
  initialValues: AwsCDKDeployStepInitialValues
  stageIdentifier: string
  onUpdate?(data: AwsCDKDeployStepInitialValues): void
  metadataMap: Required<VariableMergeServiceResponse>['metadataMap']
  variablesData: AwsCDKDeployStepInitialValues
}
...
export class AwsCDKDeployStep extends PipelineStep<AwsCDKDeployStepInitialValues> {
  ...
  protected defaultValues: AwsCDKDeployStepInitialValues = {
    identifier: '',
    name: '',
  ...
  renderStep(props: StepProps<AwsCDKDeployStepInitialValues>): JSX.Element {
    const {
      initialValues,
      stepViewType,
      inputSetData,
    ...
    if (this.isTemplatizedView(stepViewType)) {
      return (
        <AwsCDKDeployStepInputSetMode
          initialValues={initialValues}
          allowableTypes={allowableTypes}
          inputSetData={inputSetData as InputSetData<AwsCDKDeployStepInitialValues>}
        />
      )
    } else if (stepViewType === StepViewType.InputVariable) {
      const { variablesData, metadataMap } = customStepProps as AwsCDKDeployVariableStepProps
      return (
        <VariablesListTable
  ...
  validateInputSet({
    ...
    getString,
    viewType
  }: ValidateInputSetProps<AwsCDKDeployStepInitialValues>): FormikErrors<AwsCDKDeployStepInitialValues> {
    ...
    const errors = validateGenericFields({
      ...
      getString,
      viewType
    }) as FormikErrors<AwsCDKDeployStepInitialValues>
  ...
  /* eslint-disable */
  processFormData(formData: any): AwsCDKDeployStepInitialValues {
    let commandOptions
    let stackNames
    ...
    if (formData.spec.parameters && !isEmpty(formData.spec.parameters)) {
      parameters = (formData.spec?.parameters as MapValue).reduce(
        (agg: { [key: string]: string }, param: { key: string; value: string }) => ({
          ...agg,
