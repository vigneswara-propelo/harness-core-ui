...
export default function SelectDeploymentType({
  ...
  const { SSH_NG, ECS_NG, NG_SVC_ENV_REDESIGN } = useFeatureFlags()

  // Supported in NG (Next Gen - The one for which you are coding right now)
  const ngSupportedDeploymentTypes = React.useMemo(() => {
    return getNgSupportedDeploymentTypes({ SSH_NG, ECS_NG, NG_SVC_ENV_REDESIGN })
  }, [SSH_NG, ECS_NG, NG_SVC_ENV_REDESIGN])
