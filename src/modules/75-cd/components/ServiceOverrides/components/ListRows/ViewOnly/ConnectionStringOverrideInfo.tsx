...
import type { ConnectionStringsConfiguration } from 'services/cd-ng'
...
export default function ConnectionStringOverrideInfo({
  store
}: {
  store: ConnectionStringsConfiguration['store']
}): React.ReactElement {
    ...
    <Layout.Horizontal spacing={'medium'}>
      <Layout.Vertical width={180} height={40} flex={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <Layout.Horizontal spacing={'small'}>
          <Icon inline name={ConnectorIcons[store?.type as ConnectorTypes]} size={20} width={20} />
