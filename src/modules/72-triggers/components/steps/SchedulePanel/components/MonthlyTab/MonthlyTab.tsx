...
export default function MonthlyTab(props: MonthlyTabInterface): JSX.Element {
  ...
  return (
    <div className={css.monthlyTab}>
      <Layout.Vertical>
        <Text className={css.label}> {getString('triggers.schedulePanel.runOnSpecificDay')}</Text>
        <Layout.Horizontal spacing="small" style={{ alignItems: 'center', marginBottom: 'var(--spacing-medium)' }}>
          <Text className={css.label}>{getString('triggers.schedulePanel.startingWith')}</Text>
          <FormInput.Select
          ...
          <Text className={css.label}>
            {', '}
            {getString('triggers.schedulePanel.onThe')}
          </Text>
          <Toothpick
            ...
            adjoiningText={getString('triggers.schedulePanel.ofEvery')}
            endingText={getString('triggers.schedulePanel.monthsParentheses')}
          />
        </Layout.Horizontal>
        <TimeSelect
          label={getString('triggers.schedulePanel.runAt')}
          hoursValue={hours}
