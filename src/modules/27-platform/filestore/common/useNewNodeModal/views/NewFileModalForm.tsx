    ...
    try {
      if (!tempNode && editMode) {
        ...
        if (status === 'SUCCESS' && updateResponse) {
          showSuccess(getString('platform.filestore.fileSuccessSaved', { name: values.name }))
          updateCurrentNode({
            ...currentNode,
        ...
        if (response.status === 'SUCCESS') {
          if (tempNode) {
            showSuccess(getString('platform.filestore.fileSuccessCreated', { name: values.name }))
