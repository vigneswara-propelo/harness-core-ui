  ...
  const handleCreateUpdate = async (payload: VariableRequestDTO) => {
    try {
      ...
      if (response.status === 'SUCCESS') {
        ...
        showSuccess(
          getString(isEdit ? 'platform.variables.successUpdate' : 'platform.variables.successCreate', {
    ...
    <>
      ...
      <Formik<StringFormData>
        ...
        validationSchema={Yup.object().shape({
          ...
          type: Yup.string().trim().required(getString('platform.variables.validation.type')),
          valueType: Yup.string().trim().required(getString('platform.variables.validation.validation')),
          fixedValue: Yup.string().when('valueType', {
            is: valueType => valueType === Validation.FIXED,
            then: Yup.string().required(getString('platform.variables.validation.fixedValue')),
          ...
          <FormikForm data-testid="add-edit-variable" className={css.variableFormWrap}>
            <Layout.Vertical className={css.variableForm}>
              ...
              <Container padding={{ top: 'small' }}>
                <FormInput.Text name="fixedValue" label={getString('platform.variables.fixedValue')} />
              </Container>
            </Layout.Vertical>
