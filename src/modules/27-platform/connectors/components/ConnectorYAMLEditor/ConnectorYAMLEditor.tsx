...
import { isSMConnector } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
  ...
  return (
    <>
      {showLoader(isFetchingSchema, updating, !!isGitSyncEnabled) ? (
        <PageSpinner
          message={
            updating ? getString('platform.connectors.updating', { name: connector?.name }) : getString('loading')
          }
        />
      ) : null}
          ...
          <Button
            ...
            onClick={() =>
              saveYamlhandler({
                ...
                showSuccess,
                showError,
                successText: getString('platform.connectors.updatedSuccessfully')
              })
            }
            margin={{ top: 'large' }}
