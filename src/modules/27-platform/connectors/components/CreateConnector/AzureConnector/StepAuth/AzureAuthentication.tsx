...
import type { AzureFormInterface } from '@platform/connectors/interfaces/ConnectorInterface'
...
function AzureAuthentication(
  ...
  const tenantId = getString('platform.connectors.tenantId')
  const clientId = getString('platform.connectors.azure.clientId')
  const applicationId = getString('platform.connectors.azure.applicationId')
  ...
  const secretKeyOptions: SelectOption[] = [
    ...
    {
      label: getString('platform.connectors.azure.auth.certificate'),
  ...
  const validationSchema = Yup.object().shape({
    delegateType: Yup.string().required(
      getString('platform.connectors.chooseMethodForConnection', { name: Connectors.AZURE })
    ),
    ...
    secretType: Yup.string().when('delegateType', {
      is: DelegateTypes.DELEGATE_OUT_CLUSTER,
      then: Yup.string().required(getString('platform.connectors.tenantIdRequired'))
    }),
