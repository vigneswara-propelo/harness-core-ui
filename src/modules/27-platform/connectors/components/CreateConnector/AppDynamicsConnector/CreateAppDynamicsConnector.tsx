...
function UsernamePasswordAndApiClientOption(props: UsernamePasswordAndApiClientOptionProps): JSX.Element {
  ...
  const authOptions = useMemo(
    () => [
      ...
      { label: getString('platform.connectors.appD.apiClient'), value: AppDynamicsAuthType.API_CLIENT_TOKEN }
    ],
    []
  )
  ...
  const fieldProps =
    authTypeValue === AppDynamicsAuthType.API_CLIENT_TOKEN
      ? [
          { name: 'clientId', label: getString('platform.connectors.appD.clientId'), key: 'clientId' },
...
function AppDynamicsConfigStep(props: ConnectionConfigProps): JSX.Element {
  ...
  return (
    <>
      <StepDetailsHeader connectorTypeLabel={getString('platform.connectors.appdLabel')} />
      <Formik
        enableReinitialize
        initialValues={{
          ...initialValues
        ...
        validationSchema={Yup.object().shape({
          url: Yup.string().trim().required(getString('platform.connectors.appD.validation.controllerURL')),
          ...
          clientId: Yup.string()
            ...
            .when('authType', {
              ...
              then: Yup.string().required(getString('platform.connectors.appD.validation.clientId'))
            }),
          clientSecretRef: Yup.string()
            .nullable()
            .when('authType', {
              is: AppDynamicsAuthType.API_CLIENT_TOKEN,
              then: Yup.string().required(getString('platform.connectors.appD.validation.clientSecret'))
            })
        })}
        ...
        {formikProps => (
          <FormikForm className={styles.connectionForm}>
            <Layout.Vertical spacing="large" className={styles.appDContainer}>
              <FormInput.Text label={getString('platform.connectors.appD.controllerURL')} name="url" />
              <FormInput.Text label={getString('platform.connectors.appD.accountName')} name="accountName" />
