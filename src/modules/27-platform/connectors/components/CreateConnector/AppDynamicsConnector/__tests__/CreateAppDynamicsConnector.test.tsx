...
import { AppDynamicsAuthType } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
...
describe('Unit tests for createAppdConnector', () => {
  ...
  test('Ensure validation works', async () => {
    ...
    expect(getByText('platform.connectors.appD.validation.controllerURL')).not.toBeNull()

    // switch auth type
    await updateApiClientAuthType(AppDynamicsAuthType.API_CLIENT_TOKEN)
    fireEvent.click(container.querySelector('button[type="submit"]')!)
    expect(getByText('platform.connectors.appD.validation.clientId')).not.toBeNull()
    expect(getByText('platform.connectors.appD.validation.clientSecret')).not.toBeNull()
  ...
  test('Ensure if there is existing data, fields are populated', async () => {
    ...
    expect(document.body.querySelector('input[value="platform.connectors.appD.apiClient"]')).not.toBeNull()
