...
import { setupBackOffStrategyFormData } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
  ...
  return [
    {
      ...
      title: getString('platform.connectors.aws.fixedDelay')
    },
    {
      type: BackOffStrategy.EqualJitterBackoffStrategy,
      title: getString('platform.connectors.aws.equalJitter')
    },
    {
      type: BackOffStrategy.FullJitterBackoffStrategy,
      title: getString('platform.connectors.aws.fullJitter')
    }
  ]
}
...
const StepBackOffStrategy: React.FC<StepProps<StepBackOffStrategyProps> & ConnectorDetailsProps> = props => {
  ...
  const getRetryCountValidationSchema = (): Yup.NumberSchema<number | undefined> => {
    return Yup.number()
      .min(0)
      .typeError(
        getString('common.validation.fieldIsRequired', { name: getString('platform.connectors.aws.retryCount') })
      )
  }
  ...
  const getSpecValidationSchemaObject = () => {
        ...
        fixedBackoff: Yup.number()
          ...
          .typeError(
            getString('common.validation.fieldIsRequired', {
              name: getString('platform.connectors.aws.fixedBackoff')
            })
          ),
    ...
    return {
      baseDelay: Yup.number()
        ...
        .typeError(
          getString('common.validation.fieldIsRequired', {
            name: getString('platform.connectors.aws.baseDelay')
          })
        ),
      maxBackoffTime: Yup.number()
        ...
        .typeError(
          getString('common.validation.fieldIsRequired', {
            name: getString('platform.connectors.aws.maxBackoffTime')
          })
        ),
      retryCount: getRetryCountValidationSchema()
    }
  ...
  const RetryCountField = (
    <FormInput.Text
      name={'retryCount'}
      label={getString('platform.connectors.aws.retryCount')}
      placeholder={getString('platform.connectors.aws.retryCountPlaceholder')}
      inputGroup={{
        type: 'number'
      }}
    />
  ...
  return (
    <Layout.Vertical spacing="xxlarge">
      {!hideHeaderAndNavBtns && (
        <Text font={{ variation: FontVariation.H3 }}>{getString('platform.connectors.aws.awsBackOffStrategy')}</Text>
      )}
        ...
        {formikProps => (
          <FormikForm>
            <Container style={{ minHeight: 460 }}>
              ...
              >
                {getString('platform.connectors.aws.selectBackOffStrategyTypeLabel')}
              </Text>
              ...
              <Container margin={{ top: 'huge', bottom: 'medium' }}>
                {selectedBackOffStrategyType === BackOffStrategy.FixedDelayBackoffStrategy && (
                  <>
                    <FormInput.Text
                      name={'fixedBackoff'}
                      label={getString('platform.connectors.aws.fixedBackoff')}
                      placeholder={getString('platform.connectors.aws.fixedBackoffPlaceholder')}
                      inputGroup={{
                        type: 'number'
                      }}
                    />
                ...
                {(selectedBackOffStrategyType === BackOffStrategy.EqualJitterBackoffStrategy ||
                  ...
                  <>
                    <FormInput.Text
                      name={'baseDelay'}
                      label={getString('platform.connectors.aws.baseDelay')}
                      placeholder={getString('platform.connectors.aws.baseDelayPlaceholder')}
                      inputGroup={{
                        type: 'number'
                      }}
                    />
                    <FormInput.Text
                      name={'maxBackoffTime'}
                      label={getString('platform.connectors.aws.maxBackoffTime')}
                      placeholder={getString('platform.connectors.aws.maxBackoffTimePlaceholder')}
                      inputGroup={{
                        type: 'number'
                      }}
                    />
