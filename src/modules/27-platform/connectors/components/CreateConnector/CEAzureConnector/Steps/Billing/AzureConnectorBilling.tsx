...
const BillingExport: React.FC<StepProps<CEAzureDTO>> = props => {
    ...
    const FORM_INPUTS = [
      {
        ...
        label: getString('platform.connectors.ceAzure.billing.storageAccountName')
      },
      {
        name: 'subscriptionId',
        label: getString('platform.connectors.ceAzure.billing.subscriptionId'),
        placeholder: getString('platform.connectors.ceAzure.guidPlaceholder')
      },
      {
        name: 'containerName',
        label: getString('platform.connectors.ceAzure.billing.containerName')
      },
      {
        name: 'directoryName',
        label: getString('platform.connectors.ceAzure.billing.directoryName')
      },
      {
        name: 'reportName',
        ...
        <Layout.Vertical className={css.existingReportsWrapper}>
          ...
          <ul>
            <li className={css.hintsLineItem}>
              {getString('platform.connectors.ceAzure.existingExports.hints.nextStepHint1', {
                subscriptionId: prevStepData?.spec.subscriptionId
              })}
            </li>
            <li className={css.hintsLineItem}>
              {getString('platform.connectors.ceAzure.existingExports.hints.nextStepHint2')}
              <Button
                rightIcon="chevron-right"
                text={getString('platform.connectors.ceAzure.existingExports.createNewExportBtn')}
                onClick={() => {
  ...
  const getSchemaValidations = () => {
    ...
    return Yup.object().shape({
      storageAccountName: Yup.string()
        .required(getString('platform.connectors.ceAzure.validation.storageAccountName'))
        .test(
          'storageAccountName',
          getString('platform.connectors.ceAzure.billing.storageAccountNameRegexError'),
          storageAccountNameTest
        ),
      directoryName: Yup.string().required(getString('platform.connectors.ceAzure.validation.directoryName')),
      containerName: Yup.string().required(getString('platform.connectors.ceAzure.validation.containerName')),
      reportName: Yup.string().required(getString('platform.connectors.ceAzure.validation.reportName')),
  ...
  const instructionsList = [
    {
      type: 'button',
      text: 'platform.connectors.ceAzure.billing.launchAzureConsole',
      icon: 'main-share',
    ...
    {
      type: 'text',
      text: 'platform.connectors.ceAzure.billing.instructions.i1'
    },
    {
      type: 'hybrid',
      renderer: function instructionRenderer() {
        return (
          <Text font={{ variation: FontVariation.BODY }} color={Color.GREY_800}>
            {getString('platform.connectors.ceAzure.billing.instructions.i2')}
            <a href={connectorHelperUrls.ceAzureBillingExport} target="_blank" rel="noreferrer">
              {getString('platform.connectors.ceAzure.billing.instructions.i3')}
    ...
    {
      type: 'text',
      text: 'platform.connectors.ceAzure.billing.instructions.i4'
    }
  ]
  ...
  return (
    <Layout.Vertical className={css.stepContainer}>
      <Text
        ...
        margin={{ bottom: 'large' }}
      >
        {getString('platform.connectors.ceAzure.billing.heading')}
      </Text>
      <Text font={{ variation: FontVariation.BODY }} color={Color.GREY_600} padding={{ bottom: 'medium' }}>
        {getString('platform.connectors.ceAzure.billing.subHeading')}
      </Text>
