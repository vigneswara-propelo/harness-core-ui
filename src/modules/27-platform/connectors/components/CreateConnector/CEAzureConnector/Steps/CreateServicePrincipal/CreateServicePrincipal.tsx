...
const CreateServicePrincipal: React.FC<StepProps<CEAzureDTO> & CreateServicePrincipalProps> = (props): JSX.Element => {
  ...
  if (ENABLED.BILLING) {
    commands.push(
      <Commands
        comment={getString('platform.connectors.ceAzure.servicePrincipal.registerCommand')}
        command={`az ad sp create --id ${appId}`}
      />,
      <Commands
        comment={getString('platform.connectors.ceAzure.servicePrincipal.costVisibilityCmd')}
        command={`SCOPE=\`az storage account show --name ${storageAccountName} --query "id" | xargs\``}
      />,
      <Commands
        comment={getString('platform.connectors.ceAzure.servicePrincipal.assignRoleCmd')}
  ...
  if (ENABLED.VISIBILITY) {
    commands.push(
      <Commands
        comment={getString('platform.connectors.ceAzure.servicePrincipal.inventoryMgtCmd')}
      ...
      <Commands
        comment={
          ENABLED.OPTIMIZATION && ENABLED.GOVERNANCE
            ? getString('platform.connectors.ceAzure.servicePrincipal.govAndOptCmd')
            : ENABLED.OPTIMIZATION
            ? getString('platform.connectors.ceAzure.servicePrincipal.optimisationCmd')
            : getString('platform.connectors.ceAzure.servicePrincipal.governanceCmd')
        }
  ...
  return (
    <Layout.Vertical spacing="large" className={css.stepContainer}>
      ...
      <Text font={{ variation: FontVariation.H3 }} tooltipProps={{ dataTooltipId: 'servicePrincipal' }}>
        {getString('platform.connectors.ceAzure.servicePrincipal.heading')}
      </Text>
      <Text font={{ variation: FontVariation.BODY }} color={Color.GREY_800}>
        {getString('platform.connectors.ceAzure.servicePrincipal.subHeading1')}
      </Text>
      <Text font={{ variation: FontVariation.BODY }} color={Color.GREY_800}>
        {getString('platform.connectors.ceAzure.servicePrincipal.subHeading2')}
      </Text>
