...
import { buildDatadogPayload } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
...
export function DatadogConfigStep(props: ConnectionConfigProps): JSX.Element {
  ...
  return (
    <Container className={css.credentials}>
      <StepDetailsHeader connectorTypeLabel={getString('platform.connectors.title.datadog')} />
      <Formik
        formName="createDataDogConnectorsForm"
        enableReinitialize
        initialValues={{ ...initialValues }}
        validationSchema={Yup.object().shape({
          url: Yup.string().trim().required(getString('platform.connectors.datadog.urlValidation')),
          applicationKeyRef: Yup.string()
            .trim()
            .required(getString('platform.connectors.datadog.encryptedAPPKeyValidation')),
      ...
      >
        <FormikForm className={css.form}>
          <Layout.Vertical spacing="large" height={450}>
            ...
            <SecretInput
              label={getString('platform.connectors.datadog.encryptedAPPKeyLabel')}
              name="applicationKeyRef"
              scope={scope}
            />
            <SecretInput label={getString('platform.connectors.encryptedAPIKeyLabel')} name="apiKeyRef" scope={scope} />
          </Layout.Vertical>
