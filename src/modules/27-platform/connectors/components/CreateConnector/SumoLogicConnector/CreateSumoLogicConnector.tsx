...
import { buildSumoLogicPayload } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
...
export function SumoLogicConfigStep(props: ConnectionConfigProps): JSX.Element {
  ...
  return (
    <Container className={css.credentials}>
      <StepDetailsHeader connectorTypeLabel={getString('platform.connectors.title.sumologic')} />
      <Formik
        formName="crateSumoLogicConnectorsForm"
        enableReinitialize
        initialValues={{ ...initialValues }}
        validationSchema={Yup.object().shape({
          url: Yup.string().trim().required(getString('platform.connectors.sumologic.urlValidation')),
          accessIdRef: Yup.string()
            .trim()
            .required(getString('platform.connectors.sumologic.encryptedAccessIdValidation')),
          accessKeyRef: Yup.string()
            .trim()
            .required(getString('platform.connectors.sumologic.encryptedAccessKeyValidation'))
        })}
        ...
        <FormikForm className={css.form}>
          <Layout.Vertical spacing="large" height={450}>
            <FormInput.Select
              ...
              label={getString('platform.connectors.sumologic.urlLabel')}
              placeholder={loadingEndpoints ? getString('loading') : undefined}
            />
            <SecretInput label={getString('platform.connectors.sumologic.encryptedAccessIdLabel')} name="accessIdRef" />
            <SecretInput
              label={getString('platform.connectors.sumologic.encryptedAccessKeyLabel')}
              name="accessKeyRef"
            />
          </Layout.Vertical>
