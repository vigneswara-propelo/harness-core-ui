...
import { DEFAULT_PDC_AREA_LENGTH } from '@platform/connectors/constants'
    ...
    try {
      ...
      fr.onload = () => {
        try {
          ...
          showError(getString('platform.connectors.pdc.errorParsingJsonFile'))
        }
      }
      fr.readAsText(file)
    } catch (e) {
      showError(getString('platform.connectors.pdc.errorUploading'))
    }
  }
      ...
      {!fileName && !previousHosts && (
        ...
        >
          ...
          {fileName ? (
            ...
            <>
              <Text key="uploadText1">{getString('platform.connectors.pdc.hostsUpload1')}</Text>
              <Text key="uploadText2">{getString('platform.connectors.pdc.hostsUpload2')}</Text>
            </>
          )}
        </div>
      )}
      {(fileName || previousHosts) && (
        <>
          <Button
            ...
            icon="syncing"
            onClick={() => inputRef.current.click()}
            title={getString('platform.filestore.view.replaceFile')}
            text={getString('platform.filestore.view.replaceFile')}
            margin={{ left: 'small' }}
            style={{
              position: 'absolute',
              left: '600px',
          ...
          <FormInput.TextArea
            ...
            label={getString('platform.connectors.pdc.hostsJson')}
            maxLength={maxLength}
          />
        </>
      )}
