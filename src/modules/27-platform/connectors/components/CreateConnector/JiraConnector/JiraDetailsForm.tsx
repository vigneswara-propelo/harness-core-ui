...
import { setupJiraFormData, useGetHelpPanel } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
...
const JiraDetailsForm: React.FC<StepProps<JiraFormProps> & AuthenticationProps> = props => {
  ...
  const authOptions: SelectOption[] = React.useMemo(
    () => [
      {
        label: getString('platform.connectors.jira.usernameAPIKey'),
            ...
            <>
              ...
              <Layout.Vertical padding={{ top: 'large', bottom: 'large' }} width={'56%'}>
                <FormInput.Text
                  ...
                  label={getString('platform.connectors.jira.jiraUrl')}
                />
                ...
                {formik.values.authType === AuthTypes.USER_PASSWORD ? (
                  <>
                    ...
                    />
                    <SecretInput name={'passwordRef'} label={getString('platform.connectors.apiKey')} scope={scope} />
                  </>
                ) : null}
