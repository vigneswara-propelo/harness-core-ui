...
const OverviewStep: React.FC<OverviewProps> = props => {
  ...
  return (
    <Layout.Vertical className={css.stepContainer}>
      ...
      >
        {getString('platform.connectors.ceAws.overview.heading')}
      </Text>
      <div style={{ flex: 1 }}>
        <Formik<OverviewDetails>
          ...
          validationSchema={Yup.object().shape({
            ...
            awsAccountId: Yup.number()
              .typeError(getString('platform.connectors.ceAws.overview.validation.numeric'))
              .positive(getString('platform.connectors.ceAws.overview.validation.positive'))
              .required(getString('platform.connectors.ceAws.overview.validation.required'))
          })}
          onSubmit={values => {
            ...
            <FormikForm>
              ...
              <Container className={css.main}>
                ...
                <FormInput.Text
                  ...
                  name={'awsAccountId'}
                  label={getString('platform.connectors.ceAws.overview.awsAccountId')}
                  ...
                  <Layout.Vertical spacing="large">
                    <div style={{ color: 'red' }}>
                      ...
                      <span style={{ fontSize: 'var(--font-size-normal)' }}>
                        {getString('platform.connectors.ceAws.overview.alreadyExist')}
                      </span>
                    </div>
                    <div>
                      <Icon name="info" style={{ paddingRight: 5 }}></Icon>
                      {getString('platform.connectors.ceAws.overview.alreadyExistInfo', {
                        awsAccountID,
                    ...
                    <div>
                      ...
                      {getString('platform.connectors.ceAws.overview.trySuggestion')}
                      <div>
                        {getString('platform.connectors.ceAws.overview.editConnector')}{' '}
                        <Link to={routes.toConnectorDetails({ accountId, connectorId: existingConnectorId })}>
                          {existingConnectorName}
                        </Link>{' '}
                        {getString('platform.connectors.ceAws.overview.ifReq')}
                      </div>
                    </div>
                  </Layout.Vertical>
                </div>
              ...
              <Container className={css.govCloudContainer}>
                <Text className={css.govCloudLabel}>
                  {getString('platform.connectors.ceAws.overview.govCloudHeader')}
                </Text>
