...
const CostUsageStep: React.FC<StepProps<CEAwsConnectorDTO>> = props => {
  ...
  const getValidationScheme = () => {
    if (isExistingCostUsageReport) return undefined
      ...
      return Yup.object().shape({
        reportName: Yup.string().required(getString('platform.connectors.ceAws.cur.validation.reportRequired')),
        s3BucketName: Yup.string().required(getString('platform.connectors.ceAws.cur.validation.bucketRequired'))
      })
  }
  ...
  const instructionsList = [
    {
      type: 'button',
      text: 'platform.connectors.ceAws.cur.launchTemplate',
      icon: 'main-share',
    ...
    {
      type: 'text',
      text: 'platform.connectors.ceAws.cur.instructions.i1'
    },
    {
      type: 'hybrid',
        ...
        return (
          <Text font={{ variation: FontVariation.BODY }} color={Color.GREY_800}>
            ...
            <a href={connectorHelperUrls.ceAwscostUsageReportSteps} target="_blank" rel="noreferrer">
              {getString('platform.connectors.ceAws.cur.instructions.i3')}
    ...
    {
      type: 'text',
      text: 'platform.connectors.ceAws.cur.instructions.i4'
    }
  ]
  ...
  return (
    <Layout.Vertical className={css.stepContainer}>
      <Text
        ...
        margin={{ bottom: 'large' }}
      >
        {getString('platform.connectors.ceAws.cur.heading')}
      </Text>
      <Text color={Color.GREY_800} font={{ variation: FontVariation.BODY }} margin={{ bottom: 'large' }}>
        {getString('platform.connectors.ceAws.cur.subheading')}
      </Text>
            ...
            <FormikForm>
                  ...
                  <div>
                    <ul>
                      <li className={css.hintsLineItem}>
                        {getString('platform.connectors.ceAws.curExising.nextStepHint1')}
                      </li>
                      <li className={css.hintsLineItem}>
                        {getString('platform.connectors.ceAws.curExising.nextStepHint2')}
                        <Button
                          rightIcon="chevron-right"
                          text={getString('platform.connectors.ceAws.cur.createNew')}
                          onClick={() => {
              ...
              {!isExistingCostUsageReport && (
                <div>
                  <FormInput.Text
                    name={'reportName'}
                    label={getString('platform.connectors.ceAws.cur.reportName')}
                    className={css.dataFields}
                    tooltipProps={{ dataTooltipId: 'reportName' }}
                  />
                  <FormInput.Text
                    name={'s3BucketName'}
                    label={getString('platform.connectors.ceAws.cur.bucketName')}
                    className={css.dataFields}
                  ...
                  >
                    {getString('platform.connectors.ceAws.cur.noAccountLink')}
                  </a>
                </div>
              )}
