...
import { setupAwsKmsFormData } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
import {
...
const AwsKmsConfig: React.FC<StepProps<StepDetailsProps> & ConnectorDetailsProps> = props => {
  ...
  const credTypeOptions: SelectOption[] = [
    ...
    {
      label: getString('platform.connectors.awsKms.awsSTS'),
      ...
      <Formik
        ...
        validationSchema={Yup.object().shape({
          accessKey: Yup.object().when(['credType'], {
            ...
            then: Yup.object().required(getString('platform.connectors.aws.validation.accessKey'))
          }),
          secretKey: Yup.object().when(['credType'], {
            is: credentials => credentials === credTypeOptions[0].value,
            then: Yup.object().required(getString('platform.connectors.aws.validation.secretKeyRef'))
          }),
          awsArn: Yup.object().required(getString('platform.connectors.awsKms.validation.selectAWSArn')),
          region: Yup.string().trim().required(getString('platform.connectors.awsKms.validation.selectRegion')),
          ...
          externalName: Yup.string().when(['credType'], {
            ...
            then: Yup.string()
              .trim()
              .min(2, getString('platform.connectors.awsKms.validation.externalIdLengthError'))
              .max(1224, getString('platform.connectors.awsKms.validation.externalIdLengthError'))
              .matches(externalIdRegExpression, getString('platform.connectors.awsKms.validation.externalIdRegexError'))
          }),
          assumeStsRoleDuration: Yup.number().when(['credType'], {
            is: credentials => credentials === credTypeOptions[2].value,
            then: Yup.number()
              .integer(getString('platform.connectors.awsKms.validation.durationError'))
              .min(900, getString('platform.connectors.awsKms.validation.durationNumber'))
              .max(43200, getString('platform.connectors.awsKms.validation.durationNumber'))
              .typeError(getString('platform.connectors.awsKms.validation.durationError'))
          })
        })}
        onSubmit={formData => {
            ...
            <FormikForm>
              <Container style={{ minHeight: 420, maxWidth: 600 }} margin={{ top: 'medium', bottom: 'xxlarge' }}>
                ...
                {formik.values?.credType === credTypeOptions[2].value && (
                  <>
                    <FormInput.Text name="roleArn" label={getString('platform.connectors.awsKms.roleArnLabel')} />
                    <FormInput.Text name="externalName" label={getString('platform.connectors.aws.externalId')} />
                    <FormInput.Text
                      name="assumeStsRoleDuration"
                      label={getString('platform.connectors.awsKms.assumedRoleDuration')}
                    />
                  </>
                )}
