...
import { setupRancherFormData, DelegateCardInterface } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
...
import { AuthTypes, getLabelForAuthType } from '@platform/connectors/pages/connectors/utils/ConnectorHelper'
    ...
    const DelegateCards: DelegateCardInterface[] = [
      {
        ...
        info: getString('platform.connectors.rancher.delegateOutClusterInfo')
      }
    ]
    ...
    const validationSchema = Yup.object().shape({
      rancherUrl: Yup.string()
        ...
        .when('delegateType', {
          ...
          then: URLValidationSchema(getString, { requiredMessage: getString('platform.connectors.rancher.validation') })
        }),
      delegateType: Yup.string().required(
        getString('platform.connectors.chooseMethodForConnection', { name: Connectors.Rancher })
      ),
      authType: Yup.string()
        .nullable()
        .when('delegateType', {
    ...
    return loadingConnectorSecrets ? (
      ...
      <Layout.Vertical spacing="medium" className={css.secondStep}>
        ...
        <Text font={{ variation: FontVariation.H6, weight: 'semi-bold' }} color={Color.GREY_600}>
          {getString('platform.connectors.rancher.delegateSelector')}
        </Text>
        <Formik
          initialValues={{
            ...initialValues,
              ...
              <>
                <Container className={css.clusterWrapper}>
                  ...
                  {DelegateTypes.DELEGATE_OUT_CLUSTER === formikProps.values.delegateType ? (
                    <>
                      <FormInput.Text
                        label={getString('platform.connectors.rancher.rancherUrlLabel')}
