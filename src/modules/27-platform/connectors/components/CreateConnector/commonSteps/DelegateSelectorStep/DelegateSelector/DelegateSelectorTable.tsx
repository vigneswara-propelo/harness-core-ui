...
import { Container, Icon, Layout, Text, PageError, TableV2, TableProps } from '@harness/uicore'
...
import type { DelegateGroupDetailsCustom } from '@platform/connectors/components/CreateConnector/commonSteps/DelegateSelectorStep/DelegateSelector/DelegateSelector'
import { useTroubleshootModal } from '@platform/connectors/components/CreateConnector/commonSteps/DelegateSelectorStep/DelegateSelector/TroubleshootModal'
      ...
      <Layout.Vertical>
        <Layout.Horizontal flex={{ alignItems: 'center', justifyContent: 'flex-start' }} margin={{ bottom: 'xsmall' }}>
          ...
          <Text font={{ size: 'small', weight: 'bold' }}>
            {getString('platform.connectors.delegate.waitingForConnection')}
          </Text>
        </Layout.Horizontal>
        <Text
          ...
          onClick={() => onClick?.()}
        >{`(${getString('platform.delegates.delegateNotInstalled.tabs.commonProblems.troubleshoot')})`}</Text>
      </Layout.Vertical>
    )
  }
  ...
  const columns: TableProps<DelegateGroupDetailsCustom>['columns'] = useMemo(
    () => {
      const cols = [
        ...
        {
          Header: getString('platform.connectors.delegate.hearbeat').toLocaleUpperCase(),
          id: 'connectivity',
          width: '20%',
      ...
      if (showMatchesSelectorColumn) {
        cols.push({
          Header: getString('platform.connectors.delegate.matchesSelectors').toLocaleUpperCase(),
          id: 'delegateIcon',
          width: '10%',
  ...
  const getContent = (): React.ReactElement => {
    ...
    return (
      <Layout.Vertical height="100%" flex={{ align: 'center-center' }} data-name="delegateTableEmptyState">
        ...
        <Text>{getString('platform.connectors.delegate.noDelegates')}</Text>
      </Layout.Vertical>
    )
  }
  return (
