...
import { setupJenkinsFormData } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
...
const StepJenkinsAuthentication: React.FC<StepProps<StepJenkinsAuthenticationProps> & JenkinsAuthenticationProps> =
  props => {
    ...
    const authOptions: SelectOption[] = [
      {
        label: getString('platform.connectors.jenkins.usernamePasswordAPIToken'),
      ...
      <Layout.Vertical className={css.stepDetails} spacing="small">
        ...
        <Formik
          ...
          validationSchema={Yup.object().shape({
            jenkinsUrl: Yup.string().trim().required(getString('platform.connectors.jenkins.jenkinsUrlRequired')),
            ...
            bearerToken: Yup.object().when('authType', {
              ...
              then: Yup.object().required(getString('platform.connectors.jenkins.bearerTokenRequired')),
              otherwise: Yup.object().nullable()
            })
          })}
            ...
            <>
              <Layout.Vertical padding={{ top: 'large', bottom: 'large' }} className={css.secondStep} width={'64%'}>
                <FormInput.Text
                  ...
                  label={getString('platform.connectors.jenkins.jenkinsUrl')}
                />
                ...
                {formikProps.values.authType === AuthTypes.USER_PASSWORD ? (
                  <>
                    ...
                    <SecretInput
                      name={'password'}
                      label={getString('platform.connectors.jenkins.passwordAPIToken')}
                      scope={scope}
                    />
                  </>
                ) : (
