...
import { buildPrometheusPayload } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
...
export function PrometheusConfigStep(props: ConnectionConfigProps): JSX.Element {
  ...
  return (
    <Container className={css.credentials}>
      <StepDetailsHeader connectorTypeLabel={getString('platform.connectors.prometheusLabel')} />
      <Formik
        enableReinitialize
        initialValues={{ ...initialValues }}
        validationSchema={Yup.object().shape({
          url: Yup.string().trim().required(getString('platform.connectors.prometheus.urlValidation'))
        })}
        ...
        {formik => (
          <FormikForm className={css.form}>
            ...
            <Text margin={{ bottom: 'medium' }} font={{ variation: FontVariation.CARD_TITLE }}>
              {getString('platform.connectors.optionalAuthentication')}
            </Text>
            ...
            <CustomHealthKeyValueMapper
              ...
              addRowButtonLabel={getString('platform.connectors.addHeader')}
