  ...
  const handleSubmit = async (formData: ConnectorConfigDTO) => {
    try {
      ...
      const onSucessCreateOrUpdateNextStep = () => {
        props.isEditMode
          ? showSuccess(getString('platform.connectors.successfullUpdate', { name: formData.name }))
          : showSuccess(getString('platform.connectors.successfullCreate', { name: formData.name }))
    ...
    <Layout.Vertical width="60%" style={{ minHeight: 460 }} className={css.stepContainer}>
      ...
      <Formik
        ...
        validationSchema={Yup.object().shape({
          accessKey: Yup.object().required(getString('platform.connectors.aws.validation.accessKey')),
          secretKey: Yup.mixed().required(getString('platform.connectors.aws.validation.secretKeyRef'))
        })}
        ...
        {formikPros => (
          <FormikForm className={cx(css.fullHeight, css.fullHeightDivsWithFlex)}>
            <Container className={css.paddingTop8}>
              ...
              <TextReference
                name="accessKey"
                stringId="platform.connectors.aws.accessKey"
                type={(formikPros.values as any)?.accessKey?.type}
              />
              <SecretInput name="secretKey" label={getString('platform.connectors.aws.secretKey')} scope={scope} />
            </Container>
