...
import { setupVaultFormData } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
import {
  StepDetailsProps,
  ...
  return loadingFormData ? (
    ...
    <Container padding={{ top: 'medium' }}>
      <Text font={{ variation: FontVariation.H3 }} padding={{ bottom: 'xlarge' }}>
        {getString('platform.connectors.hashiCorpVault.stepTwoName')}
      </Text>
      <Formik<VaultConfigFormData>
        ...
        validationSchema={Yup.object().shape({
          ...
          sinkPath: Yup.string().when('accessType', {
            ...
            then: Yup.string().trim().required(getString('platform.connectors.hashiCorpVault.sinkPathIsRequired'))
          }),
          ...
          vaultK8sAuthRole: Yup.string().when('accessType', {
            ...
            then: Yup.string().trim().required(getString('platform.connectors.hashiCorpVault.vaultK8sAuthRoleRequired'))
          }),
          serviceAccountTokenPath: Yup.string().when('accessType', {
            is: HashiCorpVaultAccessTypes.K8s_AUTH,
            then: Yup.string().trim().required(getString('platform.connectors.hashiCorpVault.serviceAccountRequired'))
          }),
          default: Yup.boolean()
        })}
        onSubmit={formData => {
