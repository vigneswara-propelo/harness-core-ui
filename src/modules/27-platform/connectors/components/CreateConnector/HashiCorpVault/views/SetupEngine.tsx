...
import { setupEngineFormData, buildVaultPayload } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
...
const SetupEngine: React.FC<StepProps<StepDetailsProps> & ConnectorDetailsProps> = ({
  ...
  const engineTypeOptions: IOptionProps[] = [
    {
      label: getString('platform.connectors.hashiCorpVault.fetchEngines'),
      value: 'fetch'
    },
    {
      label: getString('platform.connectors.hashiCorpVault.manuallyConfigureEngine'),
      value: 'manual'
    }
  ]
    ...
    if (prevStepData) {
      ...
      try {
        ...
        const onSuccessCreateOrUpdate = () => {
          ...
          isEditMode
            ? showSuccess(getString('platform.connectors.updatedSuccessfully'))
  ...
  return (
    <>
      {loadingFormData || savingDataInProgress ? (
        <PageSpinner
          message={savingDataInProgress ? getString('platform.connectors.hashiCorpVault.saveInProgress') : undefined}
        />
      ) : null}
      <Container padding={{ top: 'medium' }}>
        <Text font={{ variation: FontVariation.H3 }} padding={{ bottom: 'xlarge' }} color={Color.BLACK}>
          {getString('platform.connectors.hashiCorpVault.setupEngine')}
        </Text>
              ...
              <FormikForm>
                <Container height={490}>
                  <FormInput.RadioGroup
                    name="engineType"
                    label={getString('platform.connectors.hashiCorpVault.secretEngine')}
                  ...
                  {formik.values['engineType'] === 'fetch' ? (
                    <Layout.Horizontal spacing="medium">
                      ...
                      <Button
                        intent="primary"
                        text={getString('platform.connectors.hashiCorpVault.fetchEngines')}
                  ...
                  {formik.values['engineType'] === 'manual' ? (
                    <Layout.Horizontal spacing="medium">
                      <FormInput.Text
                        ...
                        label={getString('platform.connectors.hashiCorpVault.engineName')}
                      />
                      <FormInput.Text
                        name="secretEngineVersion"
                        label={getString('platform.connectors.hashiCorpVault.engineVersion')}
                      />
                    </Layout.Horizontal>
                  ) : null}
                </Container>
