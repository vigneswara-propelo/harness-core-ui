...
describe('Create Secret Manager Wizard', () => {
  test('should render form', async () => {
    ...
    //Overview Step
    expect(getAllByText(container, 'platform.connectors.ceGcp.overview.projectIdLabel')[0]).toBeDefined()
    ...
    //Billing Export Page
    expect(getAllByText(container, 'platform.connectors.ceGcp.billingExport.description')[0]).toBeDefined()

    //Check if the extention opens
    expect(getAllByText(container, 'platform.connectors.ceGcp.billingExtention.heading')[0]).toBeDefined()
    ...
    expect(getAllByText(container, 'platform.connectors.ceGcp.grantPermission.step1')[0]).toBeDefined()
    expect(container).toMatchSnapshot()
    ...
    expect(getAllByText(container, 'platform.connectors.ceGcp.testConnection.heading')).toBeDefined()
  ...
  test('should show billing table for exising billing connectors', async () => {
    ...
    const { getByText } = render(
      <TestWrapper path="/account/:accountId/resources/connectors" pathParams={{ accountId: 'dummy' }}>
        <BillingExport
          name={'platform.connectors.ceGcp.billingExport.heading'}
          prevStepData={{
            name: 'gcp-con',
  ...
  test('should throw an error when connectors already exist for a given gcpProjectId', async () => {
    ...
    expect(getByText('platform.connectors.ceGcp.overview.alreadyExist')).toBeDefined()
    expect(container).toMatchSnapshot()
  })
})
