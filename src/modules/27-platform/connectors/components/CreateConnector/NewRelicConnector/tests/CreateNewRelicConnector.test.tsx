...
describe('Unit tests for createNewRelicConnector', () => {
  ...
  test('Ensure validation works', async () => {
    ...
    await waitFor(() => expect(getByText('platform.connectors.newRelic.urlFieldLabel')).not.toBeNull())
    // click submit and verify validation string is visible for user name auth type
    fireEvent.click(container.querySelector('button[type="submit"]')!)
    await waitFor(() => expect(getByText('platform.connectors.newRelic.accountIdValidation')).not.toBeNull())
    expect(getByText('platform.connectors.encryptedAPIKeyValidation')).not.toBeNull()
