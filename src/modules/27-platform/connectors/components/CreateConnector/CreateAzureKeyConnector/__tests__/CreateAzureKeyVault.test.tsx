...
describe('Create Secret Manager Wizard', () => {
  test('should be able to render first step form', async () => {
    ...
    // Step 4
    expect(getAllByText('platform.connectors.azureKeyVault.labels.setupVault')[1]).toBeTruthy()
    expect(container).toMatchSnapshot()
  })
  ...
  test('should be able to render delegate_in_cluster and go to next step', async () => {
    ...
    const userAssignedManagedIdentity = getByText('platform.connectors.azure.managedIdentities.userAssigned')
    ...
    await userEvent.click(authenticationDropdown!)
    const systemAssignedManagedIdentity = getByText('platform.connectors.azure.managedIdentities.systemAssigned')
  ...
  test('edit connector', async () => {
    ...
    // Step 4
    expect(getAllByText('platform.connectors.azureKeyVault.labels.setupVault')[1]).toBeTruthy()
    expect(getAllByText('platform.connectors.azureKeyVault.labels.setupVault').length).toEqual(2)
    expect(container).toMatchSnapshot()
  ...
  test('edit connector with manual vault details', async () => {
    ...
    // Step 4
    expect(getAllByText('platform.connectors.azureKeyVault.labels.fetchVault').length).toEqual(1)
  })
