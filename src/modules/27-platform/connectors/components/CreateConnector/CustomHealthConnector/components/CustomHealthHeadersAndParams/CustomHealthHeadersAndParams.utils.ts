...
import type { ConnectionConfigProps } from '@platform/connectors/components/CreateConnector/CommonCVConnector/constants'
...
function isHeaderOrParamValid(
  ...
  if (isTypePlainEntity(entity)) {
    ...
    if (header.value?.textField && !entity.key) {
      return { isKeyInvalid: true, validationStr: getString('platform.connectors.customHealth.keyRequired') }
    } else if (!header.value?.textField && entity.key) {
      return { isKeyInvalid: false, validationStr: getString('platform.connectors.customHealth.valueRequired') }
    }
...
export function validateHeadersAndParams(
  ...
  if (!formData.baseURL || !formData.baseURL.trim()) {
    errorsObj.baseURL = getString('platform.connectors.customHealth.baseURL')
  }
