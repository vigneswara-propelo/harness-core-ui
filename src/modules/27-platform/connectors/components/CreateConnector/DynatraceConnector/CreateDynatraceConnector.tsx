...
import { buildDynatracePayload } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
...
export function DynatraceConfigStep(props: ConnectionConfigProps): JSX.Element {
  ...
  return (
    <Container className={css.credentials}>
      <StepDetailsHeader connectorTypeLabel={getString('platform.connectors.dynatraceLabel')} />
      <Formik
        formName="createDynatraceConnectorForm"
        initialValues={{ ...initialValues }}
        validationSchema={Yup.object().shape({
          url: Yup.string().trim().required(getString('platform.connectors.dynatrace.urlValidation')),
          apiTokenRef: Yup.string().trim().required(getString('platform.connectors.apiTokenValidation'))
        })}
        ...
        {formikProps => (
          <FormikForm className={css.form}>
            ...
            <ConnectorSecretField
              ...
              secretInputProps={{ label: getString('platform.connectors.apiToken'), name: 'apiTokenRef' }}
