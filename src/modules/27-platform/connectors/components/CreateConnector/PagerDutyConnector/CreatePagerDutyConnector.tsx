...
import { buildPagerDutyPayload } from '@platform/connectors/pages/connectors/utils/ConnectorUtils'
...
export function PagerDutyConfigStep(props: ConnectionConfigProps): JSX.Element {
  ...
  useTrackEvent(ConnectorActions.CreateConnectorLoad, {
    ...
    connector_type: Connectors.PagerDuty
  })
    ...
    <Container className={css.credentials}>
      ...
      <Formik
        ...
        validationSchema={Yup.object().shape({
          apiTokenRef: Yup.string().trim().required(getString('platform.connectors.encryptedAPIKeyValidation'))
        })}
      ...
      >
        <FormikForm className={css.form}>
          <Layout.Vertical spacing="large" height={450}>
            <SecretInput label={getString('platform.connectors.encryptedAPIKeyLabel')} name="apiTokenRef" />
          </Layout.Vertical>
