...
import type { UseCreateConnectorModalReturn } from '@platform/connectors/modals/ConnectorModal/useCreateConnectorModal'
...
export const ConnectorMenuItem: React.FC<ConnectorMenuItemProps> = ({
  ...
  const [commitMsg, setCommitMsg] = useState<string>(
    `${getString('platform.connectors.confirmDeleteTitle')} ${data.connector?.name}`
  )
    ...
    return (
      <div className={'connectorDeleteDialog'}>
        <Text margin={{ bottom: 'medium' }} className={css.confirmText} title={data.connector?.name}>{`${getString(
          'platform.connectors.confirmDelete'
  ...
  const deleteHandler = async (forceDelete?: boolean): Promise<void> => {
    try {
      ...
      if (deleted) {
        showSuccess(getString('platform.connectors.deletedSuccssMessage', { name: data.connector?.name }))
