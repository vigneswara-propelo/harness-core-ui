...
export const validateDelegateSetupDetails = (
  ...
  return Yup.object().shape({
    ...
    k8sConfigDetails: Yup.object().shape({
      k8sPermissionType: Yup.string().trim().required(getString('platform.delegates.permissionRequired')),
      namespace:
        selectedPermission === k8sPermissionType.NAMESPACE_ADMIN
          ? Yup.string().trim().required(getString('platform.delegates.delegateNamespaceRequired'))
          : Yup.string().trim()
    }),
    tokenName: Yup.string().trim().required(),
    delegateType: isHelmDelegateEnabled
      ? Yup.string().required(getString('platform.delegates.delegateCreation.installerSelectionRequired'))
      : Yup.string().trim()
  })
}
