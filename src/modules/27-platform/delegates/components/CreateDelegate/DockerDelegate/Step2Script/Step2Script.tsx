      ...
      <Layout.Horizontal padding={{ top: 'small', left: 'xlarge', right: 'xlarge' }}>
        ...
        <Layout.Vertical>
          <Heading level={2} margin={{ bottom: 'large' }} color={Color.BLACK}>
            {getString('platform.delegates.delegateCreation.docker.verifyTitle')}
          </Heading>
          ...
          <Text lineClamp={2} margin={{ bottom: 'medium' }}>
            {getString('platform.delegates.delegateCreation.docker.verifyDesc2')}
          </Text>
        ...
        <div className={css.dockerDetailsContainer} style={{ display: 'none' }}>
          <Heading level={2} font={{ weight: 'bold' }} color={Color.BLACK} margin={{ bottom: 'large' }}>
            {getString('platform.delegates.delegateCreation.docker.scriptTitle')}
          </Heading>

          {getCommandBlock(getString('platform.delegates.delegateCreation.docker.scriptText'), '')}

          {getCommandBlock(getString('platform.delegates.delegateCreation.docker.scriptCommandGetIds'), 'docker ps')}

          {getCommandBlock(
            getString('platform.delegates.delegateCreation.docker.scriptSeeLogs'),
            'docker logs -f <container-id>'
          )}

          {getCommandBlock(
            getString('platform.delegates.delegateCreation.docker.scriptRunInShell'),
            'docker container exec -it <container-id> bash'
          )}

          <Text margin={{ top: 'xlarge' }} color={Color.BLACK}>
            {getString('platform.delegates.delegateCreation.docker.docLinkBefore')}
            <a
              rel="noreferrer"
              href="https://docs.harness.io/article/5ww21ewdt8-configure-delegate-proxy-settings"
              target="_blank"
            >
              {getString('platform.delegates.delegateCreation.docker.docLink')}
            </a>
          </Text>
        </div>
      </Layout.Horizontal>
