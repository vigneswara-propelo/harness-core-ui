...
describe('Token delegates component', () => {
  test('render Token delegates', async () => {
    ...
    expect(queryByText('platform.delegates.tokens.tokenNotUsedByDelegates')).not.toBeInTheDocument()
  })
  test('Open troubleshoot', async () => {
    ...
    await waitFor(() => {
      expect(getByRole('button', { name: 'platform.delegates.troubleshootOption' })).toBeInTheDocument()
    })
    await userEvent.click(getByRole('button', { name: 'platform.delegates.troubleshootOption' }))
    expect(
      queryAllByText('platform.delegates.delegateNotInstalled.tabs.commonProblems.troubleshoot')[0]
    ).toBeInTheDocument()
  })
})
