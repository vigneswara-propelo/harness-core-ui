...
describe('Delegate Tokens page', () => {
  ...
  test('Open create token modal', async () => {
    ...
    await waitFor(() => {
      expect(document.body.innerHTML).not.toContain('platform.delegates.tokens.generatedValuePlaceholder')
    })
  })
  ...
  test('Open token revoke modal', async () => {
    ...
    await waitFor(() => {
      tokenNameElement = getByText('platform.delegates.tokens.revoke')
    })
    await userEvent.click(tokenNameElement!)
    await waitFor(() => {
      expect(getByText('platform.delegates.tokens.revokeToken')).toBeInTheDocument()
    })
  })
  ...
  test('Completely revoke token', async () => {
    ...
    await waitFor(() => {
      tokenNameElement = getAllByText('platform.delegates.tokens.revoke')
    })
    await userEvent.click(tokenNameElement[0]!)
    await waitFor(() => {
      expect(getByText('platform.delegates.tokens.revokeTokenSubtitle')).toBeInTheDocument()
    })
