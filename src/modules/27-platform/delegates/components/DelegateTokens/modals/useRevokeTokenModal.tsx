    ...
    title: (
      <>
        <Heading level={3} font={{ variation: FontVariation.H3 }}>
          {getString('platform.delegates.tokens.revokeToken')}
        </Heading>
        <Text margin={{ top: 'small' }}>{getString('platform.delegates.tokens.revokeTokenSubtitle')}</Text>
      </>
    ),
    enforceFocus: false,
    style: {
        ...
        <Layout.Vertical className={css.addTokenModalContainer}>
          ...
          <Layout.Horizontal spacing="small">
            <Button intent="danger" variation={ButtonVariation.PRIMARY} onClick={() => onRevoke(hideModal)}>
              {getString('platform.delegates.tokens.revoke')}
            </Button>
