...
const CommonProblemsForDelegate: React.FC<CommonProblemsForDelegateProps> = ({ delegateType }) => {
  ...
  const saveFeedBack = () => {
    ...
    addFeedback({
      ...
      if (data.resource) {
        showSuccess(getString('platform.delegates.commandLineCreation.feedBackSaved'))
      } else {
        showError(getString('platform.delegates.commandLineCreation.feedBackNotSaved'))
      }
    })
  }
  ...
  return (
    ...
    >
      <Text font={{ variation: FontVariation.H5 }} margin={{ bottom: 'xxlarge' }}>
        {getString('platform.delegates.delegateNotInstalled.tabs.troubleshooting')}
      </Text>
      ...
      >
        <Text font={{ variation: FontVariation.H6 }}>
          {getString('platform.delegates.commandLineCreation.didDelegateComeUp')}
        </Text>
        ...
        <Button
          ...
          onClick={() => {
            ...
            setProblemSolvedOrNotText(getString('platform.delegates.commandLineCreation.delegateFixed'))
          }}
        />
        <Button
          size={ButtonSize.SMALL}
          ...
          onClick={() => {
            ...
            setProblemSolvedOrNotText(getString('platform.delegates.commandLineCreation.delegateNotFixed'))
          }}
        />
      </Layout.Horizontal>
      {problemSolvedOrNotText && (
        <Layout.Vertical spacing="none" margin={{ bottom: 'xlarge' }}>
          ...
          <Layout.Vertical spacing="none" margin={{ bottom: 'medium' }}>
            <TextArea
              placeholder={getString('platform.delegates.commandLineCreation.enterYourResponse')}
              maxLength={2048}
              autoFocus={true}
              value={feedBack}
              onChange={e => {
            ...
            <>
              ...
              >
                <a target="_blank" rel="noreferrer" href={mailtoHarness}>
                  {getString('platform.delegates.delegateNotInstalled.contactHarness')}
                </a>
              </Text>
              <Text
                margin={{ bottom: 'medium' }}
