...
const StepTestIP: React.FC<StepTestIPProps> = props => {
  ...
  const onTestIPSubmit = async (formData: any) => {
    ...
    try {
      ...
      modalErrorHandler?.showDanger(
        defaultTo((err as Error)?.message, getString('platform.authSettings.ipAddress.errorWhileValidating'))
      )
    }
  }
  ...
  if (successTestIP) {
    ...
    message = getString('platform.authSettings.ipAddress.inRange')
  } else if (failTestIP) {
    intent = Intent.DANGER
    message = getString('platform.authSettings.ipAddress.notInRange')
  }
  return (
    <Layout.Vertical spacing="xxlarge">
      ...
      <Formik<StepTestIPForm>
        ...
        validationSchema={Yup.object().shape({
          ...
          ipAddress: Yup.string().required(getString('platform.authSettings.ipAddress.required'))
        })}
      >
        {() => {
          return (
                  ...
                  <Layout.Vertical spacing="large">
                    <Container>
                      ...
                      <Layout.Vertical margin={{ bottom: 'medium' }}>
                        <FormInput.Text
                          name="testIP"
                          label={getString('platform.authSettings.ipAddress.testIPIfInRange')}
                          intent={intent}
                    ...
                    <FormInput.Text
                      disabled
                      label={getString('platform.authSettings.ipAddress.ipAddressCIDR')}
                      name="ipAddress"
                    />
                  </Layout.Vertical>
                </Container>
