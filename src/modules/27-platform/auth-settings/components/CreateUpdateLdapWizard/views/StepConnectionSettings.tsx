...
> = ({ stepData, previousStep, nextStep, updateStepData, displayName, identifier, isEdit }) => {
  ...
  const validationSchema = Yup.object().shape({
    [connectionSettingsFields.HOST]: Yup.string()
      .trim()
      .required(getString('platform.authSettings.ldap.connSettings.validateHost.required')),
    [connectionSettingsFields.PORT]: Yup.number()
      .typeError(getString('platform.authSettings.ldap.connSettings.portNumber.number'))
      .required(getString('platform.authSettings.ldap.connSettings.portNumber.required')),
    [connectionSettingsFields.MAX_REFERRAL_HOPS]: Yup.number().when([connectionSettingsFields.REFERRALS_ENABLED], {
      is: true,
      then: Yup.number()
        .typeError(getString('platform.authSettings.ldap.connSettings.validateReferralHops.number'))
        .required(getString('platform.authSettings.ldap.connSettings.validateReferralHops.required'))
        .test(
          'Positive number',
          getString('platform.authSettings.ldap.connSettings.validateReferralHops.number'),
          value => value > 0
        )
    }),
    [connectionSettingsFields.CONNECT_TIMEOUT]: Yup.number()
      .typeError(getString('platform.authSettings.ldap.connSettings.validateConnectionTimeout.number'))
      .required(getString('platform.authSettings.ldap.connSettings.validateConnectionTimeout.required')),
    [connectionSettingsFields.RESPONSE_TIMEOUT]: Yup.number()
      .typeError(getString('platform.authSettings.ldap.connSettings.validateResponseTime.number'))
      .required(getString('platform.authSettings.ldap.connSettings.validateResponseTime.required')),
    [connectionSettingsFields.BIND_DN]: Yup.string()
      .trim()
      .required(getString('platform.authSettings.ldap.connSettings.validateBindDN')),
            ...
            <FormikForm>
              <Layout.Horizontal margin={{ bottom: 'large' }}>
                <Text font={{ variation: FontVariation.H4 }}>
                  {getString('platform.authSettings.ldap.connectionSettings')}
                </Text>
              </Layout.Horizontal>
                ...
                <Layout.Horizontal>
                  <Container className={css.connectionWiderFieldSet}>
                    <FormInput.Text
                      ...
                      placeholder={getString('platform.authSettings.ldap.connSettings.hostAddress')}
                    />
                  </Container>
                </Layout.Horizontal>
                  ...
                  <Container className={css.connectionFieldSet}>
                    <Layout.Horizontal spacing="medium">
                      <FormInput.Text
                        ...
                        label={getString('platform.authSettings.ldap.connSettings.maximumReferralHops')}
                      />
                      <Container className={css.hangingFieldSet}>
                        <FormInput.CheckBox
                          label={getString('platform.authSettings.ldap.connSettings.enableReferrals')}
                ...
                <Layout.Horizontal spacing="medium">
                  <Container className={css.connectionWiderFieldSet}>
                    <Layout.Horizontal spacing="medium">
                      <FormInput.Text
                        name="connectTimeout"
                        label={getString('platform.authSettings.ldap.connSettings.connectionTimeout')}
                      />
                      <Container className={css.hangingFieldSet}>
                        <FormInput.CheckBox
                          label={getString('platform.authSettings.ldap.connSettings.recursiveSearch')}
                  ...
                  <Container className={css.connectionFieldSet}>
                    <Layout.Horizontal>
                      <FormInput.Text
                        label={getString('platform.authSettings.ldap.connSettings.responseTime')}
                ...
                <Layout.Vertical>
                  <Layout.Horizontal spacing="medium">
                    <Container className={css.connectionWiderFieldSet} padding={{ right: 'small' }}>
                      <FormInput.Text
                        name="bindDN"
                        placeholder={getString('platform.authSettings.ldap.connSettings.bindDN')}
                        label={getString('platform.authSettings.ldap.connSettings.bindDN')}
                      />
                    </Container>
                  </Layout.Horizontal>
                  <Layout.Horizontal spacing="medium">
                    <Container className={css.connectionWiderFieldSet}>
                      <FormInput.Text
                        name="bindPassword"
                        placeholder={getString('platform.authSettings.ldap.connSettings.passwordPlaceholder')}
