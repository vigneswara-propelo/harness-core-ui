  ...
  return (
    ...
    >
      ...
      <Text font={{ variation: FontVariation.H5 }} color={Color.GREY_600}>
        {getString('platform.authSettings.ldap.addGroupQueryHeading')}
      </Text>
      <Text font={{ variation: FontVariation.BODY }} width={400} className={css.addQueryDescription}>
        {getString('platform.authSettings.ldap.addGroupQueryDescription')}
      </Text>
      <Button
        text={getString('platform.authSettings.ldap.newGroupQuery')}
        icon="plus"
...
> = ({
  ...
  const groupQueryValidationSchema = Yup.object().shape({
    [groupQueryFields.BASE_DN]: Yup.string().trim().required(getString('platform.authSettings.ldap.baseDNRequired')),
    [groupQueryFields.SEARCH_FILTER]: Yup.string()
      .trim()
      .required(getString('platform.authSettings.ldap.searchFilterRequired')),
    [groupQueryFields.NAME_ATTR]: Yup.string()
      .trim()
      .required(getString('platform.authSettings.ldap.nameAttributesRequired')),
    [groupQueryFields.DESCRIPTION_ATTR]: Yup.string()
      .trim()
      .required(getString('platform.authSettings.ldap.descriptionAttributesRequired'))
  })
      ...
      >
        <FormikForm>
          <Layout.Horizontal spacing="small" flex={{ alignItems: 'center' }}>
            <QueryFormTitle title={getString('platform.authSettings.ldap.groupQueryTitle', { index: displayIndex })} />
          ...
          <Layout.Horizontal spacing="medium">
            <Container className={css.settingsForm}>
              ...
              <FormInput.Text
                className={css.queryFormField}
                label={getString('platform.authSettings.ldap.descriptionAttributes')}
    ...
    <Layout.Vertical spacing="small" padding="medium" className={customClass}>
      <Layout.Horizontal spacing="small" flex={{ alignItems: 'center' }}>
        <Text font={{ variation: FontVariation.H5 }}>
          ...
          {getString('platform.authSettings.ldap.groupQueryTitle', { index: displayIndex })}
        </Text>
      ...
      <Layout.Horizontal>
        <ul className={css.userSettingsList}>
          ...
          <li>
            <Layout.Horizontal spacing="small" flex={{ alignItems: 'center', justifyContent: 'flex-start' }}>
              <Text font={{ variation: FontVariation.BODY2 }} margin={{ right: 'xsmall' }}>
                {`${getString('platform.authSettings.ldap.descriptionAttributes')}: `}
              </Text>
    ...
    <Layout.Vertical className={cx(css.stepContainer, css.stepQueryContainer)}>
      ...
      <Layout.Horizontal margin={{ bottom: 'medium' }} className={css.alignCenter}>
        <Text className={css.fluidLabel} color={Color.BLACK}>
          {getString('platform.authSettings.ldap.setScopeForGroupQuery')}
        </Text>
        {!isSettingsListEmpty && (
          <Button
            text={getString('platform.authSettings.ldap.newGroupQuery')}
