...
const Header: React.FC<{ unitPrice: number; paymentFreq: TimeType }> = () => {
  ...
  //     ? `${getString('platform.authSettings.costCalculator.mau.perkMau')} ${getString(
  //         'platform.authSettings.costCalculator.mau.kMauFree'
  //       )}`
  //     : `${getString('platform.authSettings.costCalculator.mau.permMau')} ${getString(
  //         'platform.authSettings.costCalculator.mau.mMauFree'
  //       )}`
  // const unitPriceDescr = `${getString('platform.authSettings.unitPrice')}: ${getAmountInCurrency(
  //   CurrencyType.USD,
  //   unitPrice
  // )} ${mauUnitDecr}`
  return (
    <Layout.Vertical padding={{ bottom: 'medium' }}>
      <Text font={{ variation: FontVariation.H5 }}>{getString('platform.authSettings.costCalculator.mau.title')}</Text>
      {/* <Layout.Horizontal spacing={'small'}>
        <Text
          color={Color.PRIMARY_7}
          tooltip={getString('platform.authSettings.costCalculator.mau.mauDefinition')}
          font={{ size: 'xsmall' }}
        >
          {getString('platform.authSettings.costCalculator.mau.mau')}
        </Text> */}
...
const MAUSubscriptionInfo: React.FC<MAUSubscriptionInfoProps> = ({
  ...
  return (
    <Layout.Horizontal flex={{ justifyContent: 'space-between' }} className={css.subscriptionInfo}>
      <Item title={getString('platform.authSettings.costCalculator.currentSubscribed')} value={currentPlanDescr} />
      <Item
        title={getString('platform.authSettings.costCalculator.using')}
        value={<Text font={{ weight: 'bold' }}>{getUnitWithNumbers(usage)}</Text>}
      />
      <Item
        title={getString('platform.authSettings.recommendation')}
        value={
