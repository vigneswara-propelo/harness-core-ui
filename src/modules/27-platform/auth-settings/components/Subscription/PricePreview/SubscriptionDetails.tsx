...
const TaxLine: React.FC<{ taxAmount?: number }> = ({ taxAmount }) => {
  ...
  const taxAmountDescr = !isNil(taxAmount)
    ...
    : getString('platform.authSettings.pricePreview.calculatedNextStep')
  return (
    <Layout.Horizontal
    ...
    >
      <Text>{getString('platform.authSettings.costCalculator.tax')}</Text>
...
const PremiumSupportLine: React.FC<{ premiumSupportAmount: number }> = ({ premiumSupportAmount }) => {
  ...
  return (
    <Layout.Horizontal
      ...
      padding={{ top: 'small', bottom: 'small' }}
    >
      <Text>{getString('platform.authSettings.costCalculator.premiumSupport')}</Text>
  ...
  return (
    <Layout.Vertical padding={{ bottom: 'large' }} spacing="large">
      ...
      {isNil(subscriptionDetails.taxAmount) && (
        <Text font={{ size: 'xsmall' }}>{getString('platform.authSettings.salesTax')}</Text>
      )}
    </Layout.Vertical>
  )
}
