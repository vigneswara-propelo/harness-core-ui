...
const CheckIPForm: React.FC<CheckIPModalData> = ({ onClose }) => {
  ...
  if (successTestIP) {
    ...
    message = getString('platform.authSettings.ipAddress.aPartOfAllowlist')
  } else if (failTestIP) {
    intent = Intent.DANGER
    message = getString('platform.authSettings.ipAddress.notAPartOfAllowlist')
  }
  return (
    <Layout.Vertical padding={{ bottom: 'xxxlarge', right: 'xxxlarge', left: 'xxxlarge' }}>
      <Layout.Vertical spacing="large">
        <Text color={Color.GREY_900} font={{ size: 'medium', weight: 'semi-bold' }}>
          {getString('platform.authSettings.ipAddress.checkIPForAllowlist')}
        </Text>
        <Formik<StepTestIPForm>
          ...
          validationSchema={Yup.object().shape({
            testIP: Yup.string()
              .required(getString('platform.authSettings.ipAddress.ipRequired'))
              .matches(regexIpV4orV6, getString('platform.authSettings.ipAddress.ipInvalid'))
          })}
          onSubmit={values => {
              ...
              <Form>
                <Container className={css.form}>
                  ...
                  <Layout.Vertical margin={{ bottom: 'medium' }}>
                    <FormInput.Text
                      name="testIP"
                      label={getString('platform.authSettings.ipAddress.testIPIfPartOfAllowlist')}
                      intent={intent}
                      onChange={() => {
                  ...
                  {!showTable ? (
                    <Layout.Horizontal spacing="small">
                      <Button type="submit" intent="primary" variation={ButtonVariation.SECONDARY}>
                        {getString('platform.authSettings.ipAddress.check')}
                      </Button>
                    </Layout.Horizontal>
                  ) : (
                    <>
                      ...
                      >
                        {getString('platform.authSettings.ipAddress.allowedAsAPartOf')}
                      </Text>
                      <TableV2
                      ...
                      <Layout.Horizontal spacing="small">
                        ...
                        />
                        <Button
                          text={getString('platform.authSettings.ipAddress.checkAnotherIP')}
                          onClick={() => {
