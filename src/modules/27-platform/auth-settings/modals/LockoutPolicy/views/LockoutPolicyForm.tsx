  ...
  const handleSubmit = async (values: UserLockoutPolicy): Promise<void> => {
    ...
    try {
      ...
      /* istanbul ignore else */ if (response) {
        showSuccess(
          getString(
            editing ? 'platform.authSettings.lockoutPolicyUpdated' : 'platform.authSettings.lockoutPolicyEnabled'
          ),
          5000
        )
        onSubmit?.()
  ...
  return (
    <Layout.Vertical padding={{ left: 'huge', right: 'huge' }}>
      ...
      <Text color={Color.GREY_900} font={{ size: 'medium', weight: 'semi-bold' }} margin={{ bottom: 'xxlarge' }}>
        {getString('platform.authSettings.lockoutPolicy')}
      </Text>
      <Formik
          ...
          <FormikForm>
            <Layout.Vertical spacing="xlarge">
              <FormInput.Text
                ...
                label={getString('platform.authSettings.failedLoginsBeforeLocked')}
                inputGroup={{
                  type: 'number'
                }}
              />
            ...
            {/* <Text color={Color.GREY_500} padding={{ top: 'xsmall', bottom: 'xsmall' }}>
              {getString('platform.authSettings.notifyUsersWhenUserLocked')}
            </Text>
            <Select
              name="userGroupsToNotify"
              items={[]}
              inputProps={{ placeholder: getString('platform.authSettings.selectUserGroup') }}
            /> */}
