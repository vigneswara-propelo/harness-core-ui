  ...
  const handleSubmit = async (values: FormValues): Promise<void> => {
    ...
    try {
      ...
      /* istanbul ignore else */ if (response) {
        showSuccess(
          getString(
            editing ? 'platform.authSettings.passwordStrengthUpdated' : 'platform.authSettings.passwordStrengthEnabled'
          ),
          5000
        )
        onSubmit?.()
  ...
  return (
    <Layout.Vertical padding={{ left: 'huge', right: 'huge' }}>
      ...
      <Text color={Color.GREY_900} font={{ size: 'medium', weight: 'semi-bold' }} margin={{ bottom: 'xxlarge' }}>
        {getString('platform.authSettings.passwordStrength')}
      </Text>
      <Formik
        ...
        {({ values, setFieldValue }) => (
          <FormikForm>
            <FormInput.Text
              ...
              label={getString('platform.authSettings.minLength')}
              inputGroup={{
                type: 'number'
              }}
            />
