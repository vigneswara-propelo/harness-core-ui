...
const LockoutPolicy: React.FC<Props> = ({ loginSettings, refetchAuthSettings, canEdit, setUpdating }) => {
  ...
  const { openDialog: confirmLockoutSettings } = useConfirmationDialog({
    titleText: getString('platform.authSettings.disableLockoutPolicy'),
    contentText: getString('platform.authSettings.confirmDisableLockoutPolicy'),
      ...
      /* istanbul ignore else */ if (isConfirmed) {
        try {
          ...
          /* istanbul ignore else */ if (response) {
            refetchAuthSettings()
            showSuccess(getString('platform.authSettings.lockoutPolicyDisabled'), 5000)
          }
        } catch (e) {
  ...
  const list = [
    {
      label: getString('platform.authSettings.failedLoginsBeforeLockedAccount'),
      value: userLockoutSettings.numberOfFailedAttemptsBeforeLockout
    },
    {
      label: getString('platform.authSettings.lockoutDuration'),
      value: (
        <>
    ...
    {
      label: getString('platform.authSettings.notifyUsersWhenTheyLocked'),
      value: userLockoutSettings.notifyUser ? getString('yes') : /* istanbul ignore next */ getString('no')
    },
    {
      label: getString('platform.authSettings.notifyUsersWhenUserLocked'),
  ...
  return (
    <Collapse
      ...
      heading={
        <Switch
          label={getString('platform.authSettings.enforceLockoutPolicy')}
