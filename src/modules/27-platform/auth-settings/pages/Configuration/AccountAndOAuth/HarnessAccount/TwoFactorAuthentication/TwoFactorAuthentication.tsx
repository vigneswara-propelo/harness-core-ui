...
const TwoFactorAuthentication: React.FC<Props> = ({ twoFactorEnabled, onSuccess, canEdit, setUpdating }) => {
  ...
  const { openDialog: confirmRedirect } = useConfirmationDialog({
    titleText: getString('platform.authSettings.enforceTwoFA'),
    contentText: getString('platform.authSettings.yourAccountWillBeLockedOut'),
    confirmButtonText: getString('platform.authSettings.goToSettings'),
  ...
  const { openDialog: confirm2FAEnable } = useConfirmationDialog({
    ...
    contentText: (
      <React.Fragment>
        <Text color={Color.BLACK}>{getString('platform.authSettings.doYouWantToEnforceTwoFAForAllMembers')}</Text>
        <ol type="1" className={css.listItem}>
          <li>{getString('platform.authSettings.newMembersWillNeedToSetUpTwoFADuringSignup')}</li>
          <li>{getString('platform.authSettings.existingMembersWillReceiveAnEmailWithQRCode')}</li>
        </ol>
      </React.Fragment>
    ),
    ...
    onCloseDialog: isConfirmed => {
      /* istanbul ignore else */ if (isConfirmed) {
        submitUpdateTwoFactorAuthentication(true, getString('platform.authSettings.twoFAEnforcementEnabled'))
      }
    }
  })

  const { openDialog: confirm2FADisable } = useConfirmationDialog({
    titleText: getString('platform.authSettings.disableTwoFAEnforcement'),
    contentText: getString('platform.authSettings.sureToDisableTwoFAEnforcement'),
    confirmButtonText: getString('confirm'),
    cancelButtonText: getString('cancel'),
    onCloseDialog: isConfirmed => {
      /* istanbul ignore else */ if (isConfirmed) {
        submitUpdateTwoFactorAuthentication(false, getString('platform.authSettings.twoFAEnforcementDisabled'))
      }
    }
  })
  ...
  return (
    <Card className={css.twoFactorAuthentication}>
      <FeatureSwitch
        ...
        label={getString('platform.authSettings.enforceTwoFA')}
