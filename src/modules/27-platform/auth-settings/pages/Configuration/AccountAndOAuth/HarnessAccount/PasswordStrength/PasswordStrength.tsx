...
import { Layout, Text, Switch, Collapse, Button, ButtonVariation, useConfirmationDialog } from '@harness/uicore'
...
const PasswordStrength: React.FC<Props> = ({ loginSettings, refetchAuthSettings, canEdit, setUpdating }) => {
  ...
  const { openDialog: confirmPasswordStrengthSettings } = useConfirmationDialog({
    titleText: getString('platform.authSettings.disablePasswordStrength'),
    contentText: getString('platform.authSettings.confirmDisablePasswordStrength'),
      ...
      /* istanbul ignore else */ if (isConfirmed) {
        try {
          ...
          /* istanbul ignore else */ if (response) {
            refetchAuthSettings()
            showSuccess(getString('platform.authSettings.passwordStrengthDisabled'), 5000)
          }
        } catch (e) {
  ...
  const list = [
    getString('platform.authSettings.atLeastNChars', {
    ...
    passwordStrengthSettings.minNumberOfUppercaseCharacters
      ? getString('platform.authSettings.haveOneUppercase')
      : /* istanbul ignore next */ '',
    passwordStrengthSettings.minNumberOfLowercaseCharacters
      ? getString('platform.authSettings.haveOneLowercase')
      : /* istanbul ignore next */ '',
    passwordStrengthSettings.minNumberOfDigits
      ? getString('platform.authSettings.haveOneDigit')
      : /* istanbul ignore next */ '',
    passwordStrengthSettings.minNumberOfSpecialCharacters
      ? getString('platform.authSettings.haveOneSpecialChar')
      : /* istanbul ignore next */ ''
  ].filter(item => item)
  ...
  return (
    <Collapse
      ...
      heading={
        <Switch
          label={getString('platform.authSettings.enforcePasswordStrength')}
    ...
    >
      ...
      >
        ...
        <Text margin={{ bottom: 'xsmall' }} color={Color.BLACK}>
          {getString('platform.authSettings.passwordMustFulfillReq')}
        </Text>
        {list.map(listItem => (
