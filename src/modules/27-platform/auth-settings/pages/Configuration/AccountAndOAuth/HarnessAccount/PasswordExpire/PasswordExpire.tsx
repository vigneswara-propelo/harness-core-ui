...
const PasswordExpire: React.FC<Props> = ({ loginSettings, refetchAuthSettings, canEdit, setUpdating }) => {
  ...
  const { openDialog: confirmPasswordExpirySettings } = useConfirmationDialog({
    titleText: getString('platform.authSettings.disablePasswordExpiration'),
    contentText: getString('platform.authSettings.confirmDisablePasswordExpiration'),
      ...
      /* istanbul ignore else */ if (isConfirmed) {
        try {
          ...
          /* istanbul ignore else */ if (response) {
            refetchAuthSettings()
            showSuccess(getString('platform.authSettings.passwordExpirationDisabled'), 5000)
          }
        } catch (e) {
  ...
  const list = [
    {
      label: getString('platform.authSettings.daysBeforePasswordExpires'),
      value: passwordExpirationSettings.daysBeforePasswordExpires
    },
    {
      label: getString('platform.authSettings.daysBeforeUserNotified'),
  ...
  return (
    <Collapse
      ...
      heading={
        <Switch
          label={getString('platform.authSettings.periodicallyExpirePassword')}
    ...
    >
      ...
      >
        ...
        <Text margin={{ bottom: 'xsmall' }} color={Color.BLACK}>
          {getString('platform.authSettings.periodicallyExpirePasswordNote')}
        </Text>
