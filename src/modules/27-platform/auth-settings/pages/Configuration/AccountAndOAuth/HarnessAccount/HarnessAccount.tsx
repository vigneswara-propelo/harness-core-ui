...
const HarnessAccount: React.FC<Props> = ({
  ...
  const { openDialog: confirmUserPasswordDisable } = useConfirmationDialog({
    titleText: getString('platform.authSettings.disableUserPasswordLogin'),
    contentText: getString('platform.authSettings.confirmDisableUserPasswordLogin'),
    ...
    onCloseDialog: isConfirmed => {
      /* istanbul ignore else */ if (isConfirmed) {
        submitUserPasswordUpdate(
          ...
          getString('platform.authSettings.loginSettingsHaveBeenUpdated')
        )
      }
    }
  })
  ...
  const toggleUsernamePassword = async (e: React.FormEvent<HTMLInputElement>): Promise<void> => {
    if (userPasswordAuthMechanismEnabled && !oauthEnabled) {
      showWarning(getString('platform.authSettings.enableAtLeastOneSsoBeforeDisablingUserPasswordLogin'))
      return
    }
  ...
  return (
    <Collapse
      ...
      heading={
        <Switch
          label={getString('platform.authSettings.useHarnessUsernameAndPassword')}
