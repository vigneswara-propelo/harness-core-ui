  ...
  return (
    <Layout.Vertical spacing="none">
      <Checkbox
        ...
        label={getString('platform.authSettings.yesIamSure')}
...
const RenderColumnEnabled: Renderer<CellProps<IpAllowlistConfigResponse>> = ({ value, row, column }) => {
  ...
  const dialogExtraMessage = (): JSX.Element | undefined => {
    if (currentIP) {
      if (currentIPPartOfAllowlist === false) {
        return (
          <Text font={{ variation: FontVariation.BODY }} color={Color.GREY_500} margin={{ bottom: 'xxlarge' }}>
            {getString('platform.authSettings.ipAddress.enableIpAddressDialogWarningFail', { ipAddress: currentIP })}
          </Text>
        )
      }
      if (currentIPPartOfAllowlist === true) {
        return (
          <Text font={{ variation: FontVariation.BODY }} color={Color.GREY_500} margin={{ bottom: 'xxlarge' }}>
            {getString('platform.authSettings.ipAddress.enableIpAddressDialogWarningSuccess')}
          </Text>
        )
      }
    }
    ...
    return (
      <Text font={{ variation: FontVariation.BODY }} color={Color.GREY_500} margin={{ bottom: 'xxlarge' }}>
        {getString('platform.authSettings.ipAddress.noCurrentIpEnableIpAddressDialogWarning')}
      </Text>
    )
  }
  ...
  const { openDialog: openEnableDialog, closeDialog: closeEnableDialog } = useConfirmationDialog({
    titleText: getString('platform.authSettings.ipAddress.enableIpAddressDialogTitle'),
    contentText: getString('platform.authSettings.ipAddress.enableIpAddressDialogContent', {
...
const RenderColumnMenu: Renderer<CellProps<IpAllowlistConfigResponse>> = ({ row, column }) => {
  ...
  const { openDialog: openDeleteDialog } = useConfirmationDialog({
    contentText: getString('platform.authSettings.ipAddress.deleteIpAddressDialogContent', {
      name: ipAllowlistConfig.name
    }),
    titleText: getString('platform.authSettings.ipAddress.deleteIpAddressDialogTitle'),
      ...
      if (didConfirm && ipAllowlistConfig.identifier) {
        deleteIPAllowlist(
          ...
          },
          {
            onSuccess: () => {
              showSuccess(
                getString('platform.authSettings.ipAddress.deleteSuccessful', {
                  name: ipAllowlistConfig.name
                })
              )
              ;(column as any).refetchListingPageAPIs()
            ...
            onError: error => {
              showError(
                defaultTo(
                  error as string,
                  getString('platform.authSettings.ipAddress.errorWhileDeleting', {
                    name: ipAllowlistConfig.name
                  })
                )
              )
  ...
  return (
    <Layout.Vertical padding={{ right: 'xlarge' }}>
      ...
      {isCurrentRowValidated ? (
        <Text font={{ size: 'small' }}>{getString('platform.authSettings.ipAddress.includesYourIpAddress')}</Text>
      ) : null}
    </Layout.Vertical>
  )
}
...
const Allowlist: React.FC = () => {
  ...
  const columns: (Column<IpAllowlistConfigResponse> & {
    ...
    () => [
      ...
      {
        Header: getString('platform.authSettings.ipAddress.ipAddressCIDR'),
        id: 'ipAddress',
      ...
      {
        Header: getString('platform.authSettings.ipAddress.applicableFor'),
        id: 'applicableFor',
        width: '15%',
  ...
  return (
    <>
      ...
      <Container margin="xlarge" padding="large" background={Color.WHITE}>
        <Layout.Vertical>
          <Text font={{ variation: FontVariation.H4 }}>{getString('platform.authSettings.ipAllowlist')}</Text>
          <Text font={{ size: 'small' }}>
            {fetchingCurrentIp
              ? getString('platform.authSettings.ipAddress.determiningCurrentIp')
              : currentIP
              ? getString('platform.authSettings.ipAddress.yourIpAddressIs', { ipAddress: currentIP })
              : getString('platform.authSettings.ipAddress.unableToDetermineIp')}
          </Text>
          <Callout className={cssConfiguration.callout}>
            {getString('platform.authSettings.ipAddress.changesMayTake5Minutes')}
          </Callout>
          {rowsExist ? (
            <Layout.Horizontal margin="large" spacing="large">
              <RbacButton
                ...
                text={getString('platform.authSettings.ipAddress.addIpAddresses')}
                icon="plus"
              ...
              <RbacButton
                ...
                text={getString('platform.authSettings.ipAddress.checkIPForAllowlist')}
                permission={createPermission}
              />
            </Layout.Horizontal>
          ) : (
            <Layout.Horizontal margin="large" spacing="large" className={css.center}>
              <Card className={css.noDataCard}>
                <Layout.Vertical>
                  ...
                  >
                    {getString('platform.authSettings.ipAddress.noIPsAllowlisted')}
                  </Text>
                  <RbacButton
                    intent="primary"
                    onClick={() => {
