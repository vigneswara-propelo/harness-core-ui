...
describe('Allowlist List View', () => {
  ...
  beforeEach(async () => {
    ...
    await waitFor(() => getAllByText('platform.authSettings.ipAddress.addIpAddresses'))
  })

  test('render, click on addIPAllowlistBtn and closeButton', async () => {
    const newIpAddressButton = getByText('platform.authSettings.ipAddress.addIpAddresses')
    await act(async () => {
  ...
  test('Toggle enabled/disabled', async () => {
    ...
    await waitFor(() => getByTextFromRTL(document.body, 'platform.authSettings.yesIamSure'))
    const dialogContainer = findDialogContainer()
    expect(dialogContainer).toBeTruthy()
    const iAmSureCheckbox = queryByText(dialogContainer as HTMLElement, 'platform.authSettings.yesIamSure')
  ...
  test('Delete IP Config', async () => {
    ...
    await waitFor(() => getByTextFromRTL(document.body, 'platform.authSettings.ipAddress.deleteIpAddressDialogTitle'))
  ...
  test('Clicks on Edit IP Allowlist Config and modal opens', async () => {
    ...
    await waitFor(() => getByTextFromRTL(document.body, 'platform.authSettings.ipAddress.defineRange'))
    const form = findDialogContainer()
    expect(form).toBeTruthy()
  ...
  test('Should render allowlist with no data', async () => {
    ...
    await waitFor(() => getAllByText('platform.authSettings.ipAddress.noIPsAllowlisted'))
    expect(_getByText('platform.authSettings.ipAddress.noIPsAllowlisted')).toBeInTheDocument()
