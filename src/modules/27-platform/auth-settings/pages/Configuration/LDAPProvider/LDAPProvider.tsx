...
const LDAPProvider: React.FC<Props> = ({ authSettings, refetchAuthSettings, permissionRequest, canEdit }) => {
  ...
  const { openDialog: confirmLdapSettingsDelete } = useConfirmationDialog({
    titleText: getString('platform.authSettings.deleteLdapProvider'),
    contentText: (
      <String
        useRichText
        stringID="platform.authSettings.deleteLdapProviderDescription"
      ...
      /* istanbul ignore else */ if (isConfirmed) {
        try {
          ...
          /* istanbul ignore else */ if (deleted) {
            refetchAuthSettings()
            showSuccess(getString('platform.authSettings.ldapProviderDeleted'), 5000)
          }
        } catch (e) {
            ...
            >
              <Container margin={{ left: 'xlarge' }}>
                <Radio
                  ...
                  label={getString('platform.authSettings.loginViaLDAP')}
                  onChange={() => {
                    openLdapTestModal()
                  }}
            ...
            <Card className={css.card}>
              ...
              <Text color={Color.GREY_800} width="70%">
                {ldapSettings.disabled
                  ? getString('platform.authSettings.authorizationNotEnabled')
                  : getString('platform.authSettings.authorizationEnabled')}
              </Text>
              <Button
                text={getString('test')}
              ...
              <Popover
                ...
                content={
                  <Menu data-testid="ldap-popover-menu">
                    ...
                    />
                    <MenuItem
                      text={getString('platform.authSettings.ldap.syncUserGroups')}
                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (resource) {
                            ...
                            showError(getString('platform.authSettings.ldap.syncUserGroupsResult.fail'), 5000)
                          }
          ...
          <Card className={css.cardWithRadioBtn}>
            <Container margin={{ left: 'xlarge', top: 'xsmall' }}>
              <Radio
                ...
                label={getString('platform.authSettings.ldap.addLdap')}
