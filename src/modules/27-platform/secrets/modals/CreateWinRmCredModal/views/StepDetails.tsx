...
import { Button, Text, StepProps, FormikForm, Formik, Container, Layout } from '@harness/uicore'
  ...
  return (
    <Container padding="small" height={500}>
      <Text margin={{ bottom: 'xlarge' }} font={{ size: 'medium' }} color={Color.BLACK}>
        {getString('platform.secrets.createWinRmCredWizard.titleDetails')}
      </Text>
      <Formik<DetailsForm>
        onSubmit={values => {
        ...
        validationSchema={Yup.object().shape({
          name: Yup.string().trim().required(getString('platform.secrets.createSSHCredWizard.validName')),
          identifier: Yup.string().when('name', {
            is: val => val?.length,
            then: Yup.string()
              .trim()
              .required(getString('platform.secrets.createSSHCredWizard.validId'))
              .matches(/^(?![0-9])[0-9a-zA-Z_$]*$/, getString('platform.secrets.createSSHCredWizard.validIdRegex'))
