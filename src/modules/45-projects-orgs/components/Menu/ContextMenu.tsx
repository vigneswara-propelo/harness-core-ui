...
const ContextMenu: React.FC<ContextMenuProps> = props => {
  ...
  const handleChaos = (event: React.MouseEvent<HTMLElement, MouseEvent>): void => {
    event.stopPropagation()
    setMenuOpen?.(false)
    history.push(
      routes.toProjectOverview({
  ...
  return (
    <Menu style={{ minWidth: 'unset' }}>
      ...
      {licenseInformation['CE']?.status === LICENSE_STATE_VALUES.ACTIVE && project.modules?.includes(ModuleName.CE) ? (
        <Menu.Item
          text={
            <Layout.Horizontal spacing="xsmall">
        ...
        <Menu.Item
          text={
            <Layout.Horizontal spacing="xsmall">
              ...
              <Text color={Color.WHITE}>{getString('projectsOrgs.gotoCHAOS')}</Text>
            </Layout.Horizontal>
          }
          onClick={handleChaos}
        />
