...
import {
  getMockTriggerConfigDefaultProps,
  getMockTriggerConfigValues,
  getMockScheduleValidationSchema
} from './scheduleMockConstants'
import SchedulePanel from '../SchedulePanel'
import { scheduleTabsId } from '../components/utils'
const defaultTriggerConfigDefaultProps = getMockTriggerConfigDefaultProps({})
  ...
  return (
    <TestWrapper>
      <Formik
        ...
        validationSchema={getMockScheduleValidationSchema()}
        onSubmit={jest.fn()}
      >
        {formikProps => {
          return (
  ...
  describe('Interactivity: Schedule Tabs', () => {
    test('Minutes selection updates expression breakdown and expression', async () => {
      const { container } = render(
        <WrapperComponent
          initialValues={getMockTriggerConfigValues({
            expression: '0/5 * * * *',
