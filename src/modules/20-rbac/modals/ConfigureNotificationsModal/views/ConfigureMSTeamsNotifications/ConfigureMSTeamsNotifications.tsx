  ...
  const handleTest = async (testData: MSTeamsNotificationsData): Promise<void> => {
    ...
    try {
      ...
      if (resp.status === 'SUCCESS' && resp.data) {
        showSuccess(getString('rbac.notifications.msTestSuccess'))
        ...
        listItemRenderer={(_, index: number) =>
          expressions ? (
            <FormInput.MultiTextInput
              name={`${name}.${index}`}
              placeholder={getString('rbac.notifications.enterMicrosoftTeamsUrl')}
              label=""
  ...
  return (
    <div className={css.body}>
      <Layout.Vertical spacing="large">
        <Text>{getString('rbac.notifications.helpMSTeams')}</Text>
          ...
          {formik => {
            return (
              <FormikForm>
                ...
                <UserGroupsInput name="userGroups" label={getString('rbac.notifications.labelMSTeamsUserGroups')} />
                {props.isStep ? (
