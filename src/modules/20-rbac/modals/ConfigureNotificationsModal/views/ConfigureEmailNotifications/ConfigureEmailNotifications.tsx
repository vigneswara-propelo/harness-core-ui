        ...
        {formik => {
          return (
            <FormikForm>
              ...
              <Button
                text={getString('rbac.notifications.buttonSend')}
                onClick={event => {
  ...
  const handleTest = async (testData: EmailTestConfigData): Promise<void> => {
    ...
    try {
      ...
      if (resp.status === 'SUCCESS' && resp.data) {
        showSuccess(getString('rbac.notifications.emailTestSuccess'))
    ...
    <div className={css.body}>
      <Layout.Vertical spacing="large">
        {props.withoutHeading ? null : (
          <Heading className={css.title}>{getString('rbac.notifications.titleEmail')}</Heading>
        )}
        <Formik
          onSubmit={handleSubmit}
          validationSchema={Yup.object().shape({
        ...
        >
          {formik => {
            return (
              <FormikForm>
                <FormInput.TextArea name={'emailIds'} label={getString('rbac.notifications.emailRecipients')} />
                <UserGroupsInput name="userGroups" label={getString('rbac.notifications.labelEmailUserGroups')} />
