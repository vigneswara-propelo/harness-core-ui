...
export function useSaveInputSetV1(inputSetInfo: InputSetInfo): UseSaveInputSetReturnType {
  const {
    createInputSet,
    updateInputSet,
    inputSetResponse,
      ...
      try {
        ...
        if (isEdit) {
          if (inputSetObj.identifier) {
            response = await updateInputSet(yamlStringify(clearNullUndefined(inputSetObj)), {
              pathParams: {
          ...
          response = await createInputSet(yamlStringify(clearNullUndefined(inputSetObj)), {
            queryParams: {
              accountIdentifier: accountId,
              orgIdentifier,
              pipelineIdentifier,
        ...
        if (!isGitSyncEnabled && initialStoreMetadata.storeType !== StoreType.REMOTE) {
          ...
          onCreateSuccess !== noop ? onCreateSuccess(response) : history.goBack()
        }
      } catch (e) {
  ...
  const handleSubmit = React.useCallback(
    async (inputSetObjWithGitInfo, gitDetails?: EntityGitDetails, storeMetadata?: StoreMetadata) => {
