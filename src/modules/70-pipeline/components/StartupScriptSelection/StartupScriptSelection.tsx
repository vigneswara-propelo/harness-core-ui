...
export default function StartupScriptSelection({
  ...
  const getConnectorList = (): Array<{ scope: Scope; identifier: string }> => {
    const connectorRef = startupCommand?.store?.spec?.connectorRef
    return !isEmpty(startupCommand) && connectorRef && connectorRef !== RUNTIME_INPUT_VALUE
      ? [
          {
            scope: getScopeFromValue(connectorRef),
            identifier: getIdentifierFromValue(connectorRef)
          }
        ]
      : []
  }
