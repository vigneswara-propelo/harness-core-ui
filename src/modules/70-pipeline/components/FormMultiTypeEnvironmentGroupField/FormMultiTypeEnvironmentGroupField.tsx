...
import type { EnvironmentGroupResponseDTO, ResponsePageServiceResponse } from 'services/cd-ng'
...
export interface EnvironmentGroupReferenceFieldProps extends Omit<IFormGroupProps, 'label'> {
  name: string
...
export function MultiTypeEnvironmentGroupField(props: EnvironmentGroupReferenceFieldProps): React.ReactElement {
  const {
    name,
    style,
    createNewLabel,
  ...
  const [pagedEnvironmentGroupData, setPagedEnvironmentGroupData] = useState<ResponsePageServiceResponse>({})
  ...
  const getReferenceFieldPropsValues = getReferenceFieldProps({
    ...
    selectedEnvironmentGroups: Array.isArray(selected) ? selected : [],
    getString
  })
  ...
  return (
    <div style={style}>
      ...
      <FormGroup {...rest} labelFor={name} helperText={helperText} intent={intent}>
        <MultiTypeReferenceInput<EnvironmentGroupResponseDTO>
          name={name}
          disabled={disabled}
          referenceSelectProps={
            {
              ...
              pagination: {
                itemCount: pagedEnvironmentGroupData?.data?.totalItems || 0,
                pageSize: pagedEnvironmentGroupData?.data?.pageSize || 10,
                pageCount: pagedEnvironmentGroupData?.data?.totalPages || -1,
                pageIndex: page || 0,
          ...
          onChange={(val, _valueType, type1) => {
            if (val && type1 === MultiTypeInputType.FIXED) {
              const { record, scope } = val as unknown as { record: EnvironmentGroupResponseDTO; scope: Scope }
              const value = {
                label: record.name,
                value:
