...
function ApplicationConfigListView({
  updateStage,
  stage,
  isPropagating,
  stringsConnectors,
  ...
  handleDeleteConfig,
  editServiceOverride
}: ApplicationConfigListViewProps): JSX.Element {
  ...
  const editApplicationConfig = (store: ConnectorTypes, type: ModalViewOption): void => {
    switch (selectionType) {
      case ApplicationConfigSelectionTypes.SERVICE_OVERRIDE:
        editServiceOverride?.()
        break
      default:
            ...
            <div className={css.columnId}>
              ...
              {get(currentOption, 'store.type') === 'Harness'
                ...
                : renderConnectorField(
                    get(currentOption, 'store.spec.connectorRef'),
                    connectorName,
                    pipelineView ? color : undefined
                  )}
            </div>
...
export default ApplicationConfigListView
