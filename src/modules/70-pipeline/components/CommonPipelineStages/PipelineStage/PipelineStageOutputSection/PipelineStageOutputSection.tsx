...
import { ChildPipelineVariablesContextProvider } from '@pipeline/components/ChildPipelineVariablesContext/ChildPipelineVariablesContext'
...
export function PipelineStageOutputSection(props: PipelineStageOutputSectionProps): React.ReactElement {
  ...
  const pipelineIdentifier = get(selectedStage?.stage as PipelineStageElementConfig, 'spec.pipeline', '') as string
  const projectIdentifier = get(selectedStage?.stage as PipelineStageElementConfig, 'spec.project', '') as string
  const orgIdentifier = get(selectedStage?.stage as PipelineStageElementConfig, 'spec.org', '') as string

  const { data: childPipelineResponse } = useGetPipeline({
    pipelineIdentifier,
    queryParams: {
  ...
  const resolvedChildPipeline: PipelineInfoConfig | undefined = React.useMemo(
    () =>
      yamlParse<PipelineConfig>(defaultTo(childPipelineResponse?.data?.resolvedTemplatesPipelineYaml, ''))?.pipeline,
    [childPipelineResponse?.data?.resolvedTemplatesPipelineYaml]
  )
    ...
    <div className={css.pipelineStageOverviewWrapper}>
      ...
      <Container className={css.content}>
        ...
        <Card className={css.outputPanel} id="outputs">
          <ChildPipelineVariablesContextProvider
            childPipeline={resolvedChildPipeline}
            childPipelineMetadata={{ accountId, orgIdentifier, projectIdentifier }}
