...
import { transformValuesFieldsConfig, editViewValidateFieldsConfig } from './SemgrepStepFunctionConfigs'
import type { SemgrepStepProps, SemgrepStepData } from './SemgrepStep'
...
export const SemgrepStepBase = (
  { initialValues, onUpdate, isNewStep = true, readonly, stepViewType, allowableTypes, onChange }: SemgrepStepProps,
  formikRef: StepFormikFowardRef<SemgrepStepData>
): JSX.Element => {
  const {
    state: {
  ...
  const valuesInCorrectFormat = getInitialValuesInCorrectFormat<SemgrepStepData, SemgrepStepData>(
    initialValues,
  ...
  return (
    <Formik
      ...
      formName="SemgrepStep"
      validate={valuesToValidate => {
        const schemaValues = getFormValuesInCorrectFormat<SemgrepStepData, SemgrepStepData>(
          valuesToValidate,
      ...
      onSubmit={(_values: SemgrepStepData) => {
        const schemaValues = getFormValuesInCorrectFormat<SemgrepStepData, SemgrepStepData>(
          _values,
    ...
    >
      {(formik: FormikProps<SemgrepStepData>) => {
        // This is required
...
export const SemgrepStepBaseWithRef = React.forwardRef(SemgrepStepBase)
