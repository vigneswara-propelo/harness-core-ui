...
import { transformValuesFieldsConfig, editViewValidateFieldsConfig } from './BrakemanStepFunctionConfigs'
import type { BrakemanStepProps, BrakemanStepData } from './BrakemanStep'
...
export const BrakemanStepBase = (
  { initialValues, onUpdate, isNewStep = true, readonly, stepViewType, allowableTypes, onChange }: BrakemanStepProps,
  formikRef: StepFormikFowardRef<BrakemanStepData>
): JSX.Element => {
  const {
    state: {
  ...
  const valuesInCorrectFormat = getInitialValuesInCorrectFormat<BrakemanStepData, BrakemanStepData>(
    initialValues,
  ...
  return (
    <Formik
      ...
      formName="BrakemanStep"
      validate={valuesToValidate => {
        const schemaValues = getFormValuesInCorrectFormat<BrakemanStepData, BrakemanStepData>(
          valuesToValidate,
      ...
      onSubmit={(_values: BrakemanStepData) => {
        const schemaValues = getFormValuesInCorrectFormat<BrakemanStepData, BrakemanStepData>(
          _values,
    ...
    >
      {(formik: FormikProps<BrakemanStepData>) => {
        // This is required
...
export const BrakemanStepBaseWithRef = React.forwardRef(BrakemanStepBase)
