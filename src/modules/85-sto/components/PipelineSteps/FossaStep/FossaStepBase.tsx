...
import { transformValuesFieldsConfig, editViewValidateFieldsConfig } from './FossaStepFunctionConfigs'
import type { FossaStepProps, FossaStepData } from './FossaStep'
...
export const FossaStepBase = (
  { initialValues, onUpdate, isNewStep = true, readonly, stepViewType, allowableTypes, onChange }: FossaStepProps,
  formikRef: StepFormikFowardRef<FossaStepData>
): JSX.Element => {
  const {
    state: {
  ...
  const valuesInCorrectFormat = getInitialValuesInCorrectFormat<FossaStepData, FossaStepData>(
    initialValues,
  ...
  return (
    <Formik
      ...
      validate={valuesToValidate => {
        const schemaValues = getFormValuesInCorrectFormat<FossaStepData, FossaStepData>(
          valuesToValidate,
      ...
      onSubmit={(_values: FossaStepData) => {
        const schemaValues = getFormValuesInCorrectFormat<FossaStepData, FossaStepData>(
          _values,
    ...
    >
      {(formik: FormikProps<FossaStepData>) => {
        // This is required
...
export const FossaStepBaseWithRef = React.forwardRef(FossaStepBase)
